...
- (2026-08-29 00:23) `[frontend]` Sidebar ACP 会话超 5 个自动折叠：单个 worktree 下 ACP 会话超过阈值时只露出最新 5 个，底部显示「展开更多（+N）/收起」切换行；激活、等待用户决策或需注意力的会话豁免折叠始终露出，终端会话不参与折叠。展开状态为组件本地状态，刷新回到折叠（`frontend/src/components/Sidebar/ProjectCard.tsx`）
