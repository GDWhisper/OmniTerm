...
- (2026-07-12) `[frontend]` 修复：终端断连后黑屏、居中重连按钮不出现 — overlay 依赖全局 `connected`（由 Sidebar 每 5 秒健康检查驱动），而 blur/idle 断连会 `setConnected(false)` + `disposeTerminal()` 销毁 xterm；后端仍在运行时健康检查随即把 `connected` 拉回 `true`，overlay 条件 `!connected` 恒为 false，按钮永不显示，xterm 已销毁留下黑屏。修复：新增与全局 `connected` 解耦的 `terminalDisconnected` 状态，仅在终端 WS 真正断开/销毁时置位、`connectWs` 成功时清除；overlay 改为依赖它；同时断连后 `initTerminal` effect 跳过自动重建（避免 overlay 被立即顶替），需用户点重连（`frontend/src/stores/appStore.ts`、`frontend/src/hooks/useTerminal.ts`、`frontend/src/components/Terminal/Terminal.tsx`）
