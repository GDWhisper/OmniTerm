...
- (2026-07-08) `[frontend]` 修复点开会话时终端输入行/光标错位、屏幕大片黑屏、无法输入 — `a06eb48` 将 `createTerminal` 改为 async（动态 import addons）后，await 期间容器布局可能因 CSS 过渡（侧边栏展开）/字体 swap 而变化，但 `ResizeObserver` 在 `term.open`/`fit.fit()` **之后**才挂载，错过的尺寸变化导致 xterm 永远 fit 到错误尺寸。修复：把 `ResizeObserver` 提前到 `term.open` 之前挂上（首帧 skip，由 rAF 触发初次 fit），并加 `isCreatingRef` 防止 StrictMode 下并发 `createTerminal` 竞态、`isCancelledRef` 让 dispose 期间能中断 in-flight 创建（`frontend/src/hooks/useTerminal.ts`、`frontend/src/test/setup.ts`、`frontend/src/components/Terminal/Terminal.test.tsx`）
