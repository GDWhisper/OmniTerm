...
- (2026-07-27) `[frontend]` Sidebar ACP 会话驻留状态可视化：ACP 会话行常驻 `ACP` 徽标（复用 `status-badge-3d` 木底黑框规范），进程驻留（未释放）→绿字、已释放→灰字，不点进会话即可分辨会话类型与释放状态；Release 按钮改为仅进程驻留时显示（已释放会话无可释放对象）（`frontend/src/components/Sidebar/Sidebar.tsx`）
