...
- (2026-08-01 00:52) `[frontend]` 修复 GIT 面板 diff/commit 详情长行不换行：`.git-diff-line` 的 `white-space: pre` 使长行横向溢出被裁（容器内无滚动条，超出部分看不到），改为 `pre-wrap` 保留缩进并自动折行；`.git-diff-text` 补 `overflow-wrap: anywhere` 兜底无空格超长 token（minified 代码/base64 等）（`frontend/src/index.css`）
