...
- (2026-08-04 18:05) `[frontend]` 修复移动端 tmux 终端右侧竖向黑条 + 行尾换行字符截断：① xterm FitAddon 计算列数时固定预留 14px 桌面滚动条宽度且未扣除容器 padding，触摸设备滚动条为 overlay（不占空间），内容网格距容器右缘 ~11px 露出纯黑 `.xterm-viewport` 背景——移动端覆盖 fit 尺寸计算改为按容器实际内容盒（`clientWidth − padding`）取整列数；② 列数放大后 `.xterm-screen` 恰等于 `cols × cellWidth`，最后一列字符因字体渲染亚像素偏差（测量 cellWidth 略窄于实际字形宽度）紧贴 screen 右缘，真实设备上溢出被裁——移动端让 `.xterm-screen` 填满容器，末列获得 ~4px 缓冲；③ 隐藏 xterm 原生滚动条避免触摸时 thumb 闪现遮挡末列（`frontend/src/hooks/useTerminal.ts`、`frontend/src/index.css`）
