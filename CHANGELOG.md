...
- (2026-08-30 16:10) `[backend]` `[frontend]` `[api]` ACP 会话工作时长计时：每条 assistant 回复正文末尾新增一行「已工作 2分钟42秒」，右缘贴合气泡右缘。口径是 **agent 实际工作时长** —— turn 定稿时结算 `work_ms = wall_ms - wait_ms`，等人工审批的挂起时长单列 `wait_ms`、不计入工作（悬停该行可见），审批无人应答由 reaper 30 分钟兜底回收，时长不再虚增。`sessions` 表加 `work_ms/wait_ms/turn_count/last_turn_at` 增量列（写时结算，读会话列表零聚合成本，跨进程回收/重启不清零），`chat_messages` 加 `duration_ms/wait_ms`（NULL=未知，区别于 0）；`prompt_done` WS 帧新增 `duration` 字段，耗时在定稿那一刻即显示，不必等刷新读库。迁移前的历史行无数据 → 前端整行不渲染，不出现 `0秒`/`NaN`。单位字形由 `Intl` unit-narrow 按界面语言给出（中文「2分钟42秒」/英文「2m42s」），代码内不硬编码「分/秒」；时长只有后端一套真相，流式期间不做「计时中」占位（`migrations/20260830_add_work_time.sql`、`src/acp/turn_accumulator.rs`、`src/acp/client.rs`、`src/acp/reaper.rs`、`src/api/sessions.rs`、`src/ws/acp.rs`、`frontend/src/components/Chat/ChatMessage.tsx`、`frontend/src/utils/formatTime.ts`）
