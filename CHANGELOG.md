...
- (2026-08-12 07:35) `[backend]` `[frontend]` 修复「密码验证关闭时手动开启」体验：点击开启开关不再立即被踢到全屏登录页（根因：`/auth/check` 在开关关闭时不返回 `needs_setup`，前端误判为「已有密码」而直接翻转开关，而 auth 关闭期间并无有效会话 cookie，下一个受保护请求 401 即被登出）。现改为在设置界面弹窗就地设置：未设过密码时展示「新密码 + 确认新密码」（校验一致后提交，随后触发重新登入）；已设过密码时展示「当前密码」验证（通过后保持登录，不跳转）。顺带修复 `client.ts` 对 `/auth/login`、`/auth/setup`、`/auth/change-password` 的 401（密码错误属业务错误而非会话过期）不再误触发登出跳转（`src/api/auth.rs`、`frontend/src/components/Settings/AuthSection.tsx`、`frontend/src/api/client.ts`、两个 `translation.json`）
