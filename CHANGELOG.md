...
- (2026-07-22) `[frontend]` ACP 聊天流在「发送 prompt → LLM 首 token」之间新增「思考中…」占位（pulsing dots，尊重 `pixelAnimationsEnabled` 开关），消除用户消息上屏后到 agent 首次输出前的静默空白（`frontend/src/components/Chat/ChatView.tsx`、`frontend/src/index.css`、`frontend/src/locales/{en,zh}/translation.json`）
