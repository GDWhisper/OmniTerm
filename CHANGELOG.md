...
- (2026-08-01 00:15) `[frontend]` ACP 聊天气泡标识行（USER/agent 名）视觉增强：颜色由 `--text-faint` 提升至 `--text-secondary` 并加粗（600），配浅色徽章底（`--bg-elevated` + `--border-subtle` 边框 + 2px 圆角），与气泡区隔更明显；hover 时间小字保持次级（faint）层级（`frontend/src/components/Chat/ChatMessage.tsx`）
...
- (2026-08-01 02:10) `[frontend]` 修复 GIT 面板 diff/commit 详情长行不换行：`.git-diff-line` 的 `white-space: pre` 使长行横向溢出被裁（容器内无滚动条，超出部分看不到），改为 `pre-wrap` 保留缩进并自动折行；`.git-diff-text` 补 `overflow-wrap: anywhere` 兜底无空格超长 token（minified 代码/base64 等）（`frontend/src/index.css`）
