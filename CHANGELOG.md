...
- (2026-07-22) `[frontend]` 修复「恢复会话后刷新仍丢历史」：根因是 `replay_end` 立即 sync，而 agent 的重放 `session_update` 可能在 `replay_end` 之后才异步到达（ACP 协议顺序不保证），导致写入不完整历史。改为稳定窗口策略——`replay_end` 后启动 1s 定时器，期间每收到新 `session_update`（文本/思考/工具/计划）就重置定时器，直到连续 1s 无新帧再 `syncToDb` 写库（`frontend/src/hooks/useAcpChat.ts`）
