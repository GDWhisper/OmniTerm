...
- (2026-08-30 14:55) `[frontend]` 终端重连遮罩改半透明：原遮罩 85% 不透明度几乎不透光，断连时看不到下方终端的残留画面，只知道「黑了」。降到 45%（`rgba(18, 20, 26, 0.45)`，中性暗色、不随主题染色），终端内容可透出、重连按钮仍清晰（`frontend/src/components/Terminal/Terminal.tsx`）
