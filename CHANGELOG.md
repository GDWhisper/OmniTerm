...
- (2026-09-23 09:07) `[backend]` `[frontend]` `[api]` 修复 pty 会话在 opencode 等开启鼠标上报的 TUI 里滚轮完全失效（tmux 会话正常）：TUI 启动时发的鼠标上报模式序列（DECSET 1000/1002/1003 + 编码 1005/1006）被 cell_frame 架构吞掉（raw 流只喂后端 VT 不转发），前端 xterm 的 mouseTrackingMode 恒为 none——滚轮既不会被编成 SGR 鼠标上报送给 TUI，wheel 的鼠标协议放行分支也永不触发，落进本地空 scrollback 彻底无动作。现与 bracketed paste 中继同型补上鼠标上报模式中继：cell_frame 所有帧携带 `mouse_mode`（none/press/drag/motion）与 `mouse_encoding`（default/utf8/sgr），前端与 xterm 实际态不一致时写 DECSET 同步其解析态（会话切换 reset 后首帧自愈），滚轮放行分支随之生效、TUI 恢复自身视口滚动；alacritty 不跟踪的 DECSET 9（X10）/1015（urxvt 编码）显式降级为未开启（`src/engine/pty/{frame,vt}.rs`、`frontend/src/hooks/{useCellFrame,useTerminal}.ts` 及同目录测试）
...
- (2026-09-21 23:35) `[frontend]` 「在此打开终端」确认弹窗新增「创建新项目并打开终端」备选：此前浏览目录被某个项目覆盖（当前工作区内的子目录或其它已打开项目的前缀）时只能把终端挂到该项目，想以该目录本身为根独立建项目得退回侧栏走一遍新建项目流程。现确认弹窗展示的目录在侧栏没有同根项目时出现该按钮，一键以展示目录为根创建项目并在其下打开终端（项目名默认取目录名；后端 409 already_covered——同仓库 worktree 等前端前缀探测看不到的覆盖——时回退挂到覆盖项目，与原主路径等价）。创建项目/建会话的共享步骤收敛到 `openTerminal.ts`，与无归属引导弹窗（`OpenTerminalDialog`）同一份实现（`frontend/src/components/FileManager/{OpenTerminalConfirmDialog,OpenTerminalDialog,FileManager,openTerminal}.tsx`、`frontend/src/utils/path.ts`）
...
- (2026-09-22 04:00) `[backend]` 修复 tmux 控制连接子进程退出后留僵尸（现场 10 个 `defunct tmux: client`、最久 10 天，父进程均为后端本体）：`ControlModeClient`（每条 tmux 终端 WS 的活动监控连接 `tmux -C attach-session`）的 `Child` 句柄只有 `stop()` 会 `wait()`，而 tmux 会话被外部 kill 时子进程**自行退出**、无人调用 `stop()`——句柄滞留在 monitor map 里，进程在 `/proc` 留僵尸；`Drop` 也只 `start_kill()` 不 `wait()`（tokio `Child` drop 不收割）。现改为**常驻收割任务独占 `Child` 句柄**（`select!(child.wait(), kill 指令)`）：无论自然退出、SIGHUP、`stop`/`Drop` 强杀还是被 drop 后的孤儿，都恰好 `wait()` 一次；退出码经 watch 通道供 `is_alive`/`stop` 诊断。与 pty 引擎、ACP terminal、ACP agent 三条既有收割路径对齐为同一模式（全库 spawn 路径审计后确认这是唯一结构缺口）。回归测试用假 tmux 客户端驱动同一段生命周期代码（本环境容器无 devpts、tmux server 起不来；RED 已用「模拟无人 wait」复证）（`src/engine/tmux/control_mode.rs`）
- (2026-09-22 03:10) `[backend]` 修复 ACP 探针连接测试 15s 超时时的 **pid 自报文件残留**：`test_agent` / `test_agent_raw` 的超时会 drop 掉 `spawn_and_connect` 的外层 future，此前只有「外层还活着」的错误分支会清理 `/tmp/omniterm-acp-*.pid`，被 drop 的路径（含 abort 兜底终止的任务）不清——每次超时残留一个几十字节文件。现在 pid 文件由 `agent_proc::PidFileCleanup` RAII 守卫随连接任务终结（返回 / abort / panic）统一删除（幂等；成功路径本就是读后即删）。两个探针路由的超时分支同时补 WARN 留痕。进程泄漏无此问题：Phase 1 的 D4 abort 兜底在外层 future 释放 `abort_tx` 时即 abort 连接任务，crate 的 `ChildGuard::drop` 完成进程组击杀——回归测试（fake agent hang 模式）钉住该契约（`src/acp/agent_proc.rs`、`src/acp/client.rs`、`src/acp/fake_agent_tests.rs`、`src/api/agents.rs`）
- (2026-09-22 02:30) `[backend]` 修复「恢复 ACP 会话后后端 CPU 空转」的**根因**：`futures` 0.3.33 的 `FuturesUnordered` 私有 `waker_ref` 存在 waker 身份缺陷（rust-lang/futures-rs#3032，cloned waker 与原始 waker 身份不一致时轮询方被立即重唤醒，形成 poll → re-arm → re-poll 紧密循环），作用在 `agent-client-protocol` 1.3.0 经 async-process 2.5.0 的 pidfd 等待路径（`wait_for_child → Child::status → try_wait`，`Async<pidfd>` + level-triggered 源被 tokio 调度器直接轮询）上即表现为连接 actor 空转烧满一核。ACP SDK 上游四格实验证实「1.3.0 + futures 0.3.33 = 空转 1 核 / +0.3.34 = 修复 / SDK 2.0 起不复现」（rust-sdk#254）。升级 futures 套件 0.3.33 → 0.3.34（`futures-util` 声明同步加地板 "0.3.34"，防未来解析回退）；同时把 `agent-client-protocol` 声明从 "1.2" 对齐为实际锁定的 "1.3"。与 09-21 的 killpg 止血构成双层防线：0.3.34 修「连接层不再空转」，killpg 修「teardown 不依赖 crate 内部路径健康」。ACP 2.x 升级评估后暂缓（破坏性大版本，收益与根因修复不重叠，见计划 Phase 3 决策记录）（`Cargo.toml`、`Cargo.lock`）
- (2026-09-21 19:20) `[backend]` 修复恢复 ACP 会话后后端 CPU 持续 ~150% 空转、且 omniterm 侧无直接手段终止 agent 进程的结构性缺陷：`AcpClient::shutdown()` 只发优雅信号，杀 agent 进程实际依赖 `agent-client-protocol` 内部 task_actor 结束后 `ChildGuard::drop` 的 killpg——连接 poll 卡死时该路径永远走不到（release/delete/archive/reaper 注释却声称「强制杀进程」）。现在 teardown 显式 **killpg 杀 agent 进程组**（顺序：`alive=false` → `mark_prompt_idle` → 终端回收 → killpg → 优雅 signal → abort 兜底），kill 使 crate 内部 pidfd 等待路径的 `try_wait` 立即返回退出状态，从根上打破空转；agent pid 由 spawn wrapper 的 `echo $$ > <pid 文件>` 自报（`/proc` 子进程 diff 兜底，仅 Linux），kill 前校验 pid 归属防 pid 复用误杀，孙进程（npx wrapper launcher 场景）随进程组一并击杀；pid 不可得时 WARN 留痕并退化为原行为。顺带重构：`spawn_and_connect` / `spawn_and_load` 的 272 行逐字重复提取为公共构造核 `spawn_with_session`（差异收敛为 `SessionMode` 枚举），新增 `src/acp/agent_proc.rs`（pid 捕获 / 进程组击杀 + 24 个单测）（`src/acp/agent_proc.rs`、`src/acp/client.rs`、`src/acp/mod.rs`）
- (2026-09-21 18:32) `[backend]` 修复 ACP turn 定稿命令在 writer 信号通道饱和时被静默丢弃：`EndTurn`（非幂等的定稿跃迁）曾与可丢的 `Flush` 防抖信号共用一条 `try_send` 通道——高帧率折叠能在 writer 一次 SQLite 写内塞满 256 深度（万级帧率下约 25ms 停滞即够），定稿命令届时静默失败，该 turn 的会话级记账（`turn_count` / `work_ms` / `wait_ms`）与消息行定稿（`chat_messages.status` 永久停在 `streaming`、`duration_ms` 为 NULL）同时丢失且无日志，而界面仍显示耗时（`turn_timing` 读发送前写好的 `last_timing`），DB 与界面静默分叉。`EndTurn` 改走独立无界可靠通道（writer 死亡时 send 失败记 WARN，不再静默），writer 侧优先消费该通道避免关闭竞态丢缓冲，定稿时顺手清空已排队的 `Flush` 信号避免一次内容相同的冗余写；回归测试钉住「信号通道塞满时 EndTurn 仍恰好送达一次」（`src/acp/turn_accumulator.rs`）
...
- (2026-09-21 15:00) `[frontend]` 修复 ACP 气泡底部「已工作 / 工具约 / 估算 t/s」三项读数的三个缺陷：① 工具时长几乎没有统计——每次 WS 重连（移动端关一次浏览器再回来必现）都把本地已观测到的工具并集整体清空，工具正好跨越重连点时那段已执行时间也永久测不到，实测两个 bash 调用只报「工具约 <1秒」；② 单位换行不跟随数字——元信息行是单个文本节点且样式允许任意字符间断行，中文无空格，窄屏下「5分钟」/「33秒」、「工具约」/「<1秒」、「估算」/「62.1 t/s」三处都被拆开；③ 工具调用期间 t/s 逐渐跌落——生成速度的分母只扣除「工具窗口内没有任何输出」的工具段，一旦工具执行期间流出思考/正文，整段开放并集被算回生成时间，分母随工具执行持续增长，看起来像模型越跑越慢。改为：已闭合的工具段跨重连保留（新增 `pureToolMsAtObs` 基线，分母只扣本观测窗口内的工具时间，不重复扣也不扣成负数）；工具并集内**首次出现输出**时把「工具起点→此刻」封口为纯工具时间，生成计时钟在此暂停直到下次流式吐字，封口之后仍在工具内的区间才算生成时间；三个读数各自一个 `white-space:nowrap` 段，换行只发生在段间（实时路径仍走定时器直写 DOM，不因此引入每秒 setState）。仍开放的并集在重连时照旧丢弃（离线段无法归因，不虚构）；「只有显式 `in_progress`/`running` 才起表」的口径不变，故对从不发显式执行状态的 agent 仍记 0 工具时间（`frontend/src/utils/turnClock.ts`、`frontend/src/components/Chat/ChatMessage.tsx`、`frontend/src/utils/turnClock.test.ts`、`frontend/src/components/Chat/ChatMessage.metarow.test.tsx`）

## [0.2.23] - 2026-09-20

### Added

- (2026-09-14 02:00) `[frontend]` 终端自动重连（聚焦页面才重连）：此前终端 WS 一断（后端重启/网络抖动/失焦与空闲主动拆除）就盖遮罩逼用户手点「重连」，切走久了回来总要被打断一次。现在 `useTerminal` 内置自动重连引擎——意外断开在页面可见时按指数退避自动重试（1→2→4→…→30s 封顶，同 ACP 聊天侧节奏，成功归零），页面隐藏期间零尝试（含到期的定时器直接跳过），用户回来（切回标签/窗口聚焦/任意鼠标键盘活动）立即重连并重置退避；blur/idle 主动拆除的省资源语义不变，拆除后由引擎在回归时自愈（整端重建），桌面端自动重建跳过一次 autoFocus 不抢聊天面板焦点。意外断开且 xterm 存活时走廉价 WS 重连（不重建终端实例）；重连期间遮罩显示「正在自动重连…」状态行，手动「重连」按钮保留为立即兜底（重连按钮文案顺带 i18n 化，原硬编码中文）。`Terminal.tsx` 挂载 effect 的 `terminalDisconnected` 移出依赖数组（body 条件仍读）——flag 翻转触发 cleanup 会把刚布好的重试连带拆毁。终端 reconnect 测试脚手架抽共享 `Terminal.testUtils.ts` + `fakeXterm.ts`（reconnect-flow / autoReconnect 共用），新增 autoReconnect 行为测试 5 条（1s 自愈/退避封顶归零/隐藏门控/空闲拆除活动自愈+不抢焦点/手动点击即时生效且取消排队）（`frontend/src/hooks/useTerminal.ts`、`frontend/src/components/Terminal/Terminal.tsx`、`frontend/src/components/Terminal/{Terminal.testUtils,fakeXterm}.ts`、`frontend/src/components/Terminal/Terminal.{autoReconnect,reconnect,reconnect-flow}.test.tsx`、`frontend/src/locales/{zh,en}/translation.json`）
- (2026-09-13 15:40) `[backend]` `[frontend]` 已结束 ACP 会话的底部配置栏（Model/Thinking/Mode/Config）置灰只读展示：此前会话被回收/释放后重新打开时配置栏整体消失，无法回看「这个会话当时用的什么配置」。后端把最后一次已知的完整 `configOptions` 快照持久化到 `sessions.config_options_json`（新 migration；ACP §12.5 全量覆盖语义，写入收口 `config_prefs::persist_config_snapshot`，attach/load/set_config_option/agent 推送四个入口统一落库，空集合跳过 + 64KB 上限），`GET /messages` 响应新增 `configOptions` + `agentLive` 字段；前端 hydrate 注入快照（`chatStore.configReadOnly`），配置栏样式与活跃态一致、整体 opacity 0.5 置灰、下拉 disabled 并带 tooltip——`set_config_option` 对无活 agent 是静默 no-op，禁用交互是必须项；发送消息自动恢复后由 live/replay 配置帧接管，自动解除置灰。配置快照解析与 live 帧共用 `parseConfigOptions` 单一真源；删除 store 上零调用的独立 `setConfigOptions` action（绕过只读不变式）（`migrations/20260913_add_config_options_snapshot.sql`、`src/acp/{config_prefs,client}.rs`、`src/api/sessions.rs`、`frontend/src/stores/chatStore.ts`、`frontend/src/hooks/useAcpChat.ts`、`frontend/src/components/Chat/{ChatView,ConfigToolbar}.tsx`）
- (2026-09-12 23:58) `[frontend]` ACP 会话新增「上次输入」悬浮卡片：消息区顶部居中悬浮（pixel-float 浮层 + hover 提亮 + 硬阴影按压反馈，不占布局），单行展示最近一次已送达的用户输入（fit-content 限宽，桌面 60% / 移动 88%，超宽 ellipsis，完整内容 hover title 查看；多行压平，纯附件显示占位）；点击把该消息气泡滚到卡片下方并短暂 accent 描边 + ring 闪烁聚焦（重复点击动画重放）。卡片显隐按气泡相对视口位置三分——在视口内、或被用户上翻越过进入更早历史（气泡沉到视口下方）均收起，仅当气泡升出视口顶缘（正在阅读其后的回复，顶缘 24px 残条视同已被卡片盖住）时显示，作为「你最后问了什么」的参照；滚动 / 消息变化 / 容器尺寸变化（ResizeObserver）三条路径重测。断连留痕的 undelivered 消息从未真正发往 agent，不算一次输入，不作展示与跳转目标；会话尚无用户输入时不渲染。消息行加 `data-chat-msg-id` 锚点、`ChatMessageView` 加 `highlighted` prop（仅目标气泡为 true，memo 浅比较不受影响）（`frontend/src/components/Chat/{ChatView,ChatMessage}.tsx`、`frontend/src/index.css`、`frontend/src/locales/{zh,en}/translation.json`）
...
- (2026-09-20 21:40) `[frontend]` ACP 上下文用量指示从底部配置栏搬到聊天面板标题栏（模式徽章与 LIVE/DEAD 之间），并修复其「时有时无」：此前用量圆环与配置控制项混排在同一行，一是窄屏下与三个配置值争宽度（移动端被迫砍掉费用文本）、二是只报用量不发配置项的 agent 会被渲染成一条只有圆环的「配置栏」；用量本质是会话级状态，与 LIVE/DEAD 同类，现改为标题栏内的木底状态徽章（`.title-bar-badge` 内嵌 3D 立体，圆环配色改为木底专用的半透明黑轨道 + 米色进度，超 80% 用浅红），明细浮层由向上弹改为向下弹（标题栏贴在面板顶缘，向上会被裁切）并锚定徽章本身。闪烁根因是会话重放会把该会话的内存状态整体清空再按重放帧回填，而用量由 agent 按 turn 推送、不随重放下发——现与 agent 名/能力支持等「不随重放下发」的字段一同跨重放保留（重放帧真带用量时仍以新值为准），不再每次重放后归零。移动端标题栏只显示圆环 + 百分比，费用文本让位（约 40px）；桌面端完整（`frontend/src/components/Chat/UsageIndicator.tsx` 新建、`frontend/src/components/Chat/{ConfigToolbar,ChatView}.tsx`、`frontend/src/stores/chatStore.ts`、`docs/reference/user-testing.md`）
- (2026-09-14 16:40) `[frontend]` ACP 移动端底部配置栏收纳：窄屏下 agent 下发的配置项（权限模式/模型/思考等级/模型参数等）超过 3 个时 flex-wrap 会叠成 2~3 行、挤占聊天视口约 50~70px。现仅移动端切分——行内只保留 权限模式 > 模型 > 思考等级 三项（按类别优先级、每类至多第一个，agent 未下发的类别自动缺席；只发非这三类的会话提升排序第一项兜底，避免只剩一个空入口），其余全部收进「高级 N」面板（N 为收纳数量，不点开也知道里面有货）。面板锚在配置栏上方、木纹标题栏 + 关闭按钮 + 统一滚动容器（最大高度 320px），每项一行「名称 + 当前值」、点行就地展开选项列表（不用二级浮层，规避选项列表嵌进滚动面板被裁切/二级滚动），点选即生效并收起该行；关闭契约与既有配置下拉一致（外部点击 / Esc），两者互斥。配置项 ≤3 个时不出现收纳按钮；已结束（置灰）会话的收纳按钮随其他按钮一同 disabled。顺带移动端紧凑化：行内按钮去掉类别前缀、当前值超宽省略、触摸目标提到 30px（原约 20px）；单行由样式契约保证——配置栏 `flex-wrap: nowrap` + 触发器 `min-width: 0`（弹性子项默认 `min-width: auto`，当前值 nowrap 时不显式给 0 会拒绝收缩、整行换行成两行，实测 360px 复现）+ 三个行内项等分剩余宽度（长值名截断最多，短值如 Ask/Off 不被牺牲）。窄屏（360px）实测：无用量指示时权限模式值完整可见、思考等级与模型名按省略号截断；agent 上报用量时圆环+百分比保留、费用文本让位（约 40px），保证三项仍有约 8 字符宽度。桌面端渲染路径不变（不切分、wrap 原行为）。只读快照 tooltip 的硬编码英文本地化（`frontend/src/components/Chat/ConfigToolbar.tsx`、`frontend/src/locales/{zh,en}/translation.json`、`docs/reference/user-testing.md`）
...
- (2026-09-12 00:24) `[infra]` 仓库根引入 `pnpm-workspace.yaml`（packages: frontend，并登记 ignoredBuiltDependencies: onnxruntime-node/protobufjs/sharp——三者构建脚本被跳过是长期现状），把 pnpm 的 workspace 锚定进仓库，根治「从 frontend/ 向上递归误认 `~/` 全局工具 workspace」的历史坑：该坑在 pnpm 12 的 run 前依赖校验下升级为 `ERR_PNPM_IGNORED_BUILDS` 硬报错，`dev.sh` 拉前端直接失败。`frontend/pnpm-lock.yaml` git mv 到仓库根（workspace 单一 lockfile，importer 仅 frontend、锁版本不变）；各 worktree 合并后一次性 `rm -rf frontend/node_modules && pnpm install`（全局 store 硬链接重建，不重新下载）。Dockerfile 前端阶段改为与仓库一致的 workspace 结构（锁版本确定性保留），新增 `.dockerignore` 堵住「宿主 glibc node_modules 整体拷进 alpine 镜像」的隐性污染；CI 三处 `cache-dependency-path` 同步改根路径。安装命令从 `cd frontend && pnpm install --ignore-workspace` 简化为仓库根裸 `pnpm install`（`docs/workflows/worktree-setup.md` 已更新）
- (2026-09-11 15:30) `[frontend]` 终端引擎选择权收敛到一个设置项，并把 tmux 立为默认：pty 引擎仍在 beta 期，此前创建会话弹窗默认高亮 pty、文件管理器「在此打开终端」更是写死 pty，等于把未定型的引擎摆在首选位。现「设置 → 终端」新增**默认引擎**（卡片/按钮序 tmux 在左、pty 居后带 BETA 角标），创建会话弹窗的初始高亮与「在此打开终端」（含「挂当前项目」「新建项目」两条分支）全部按它取值；宿主探测无复用器时该选项禁用并说明原因，实际落 pty。原「记住上次使用的引擎」并入该设置——弹窗内**显式点选**才写回默认值，不再有第二层优先级链（卡片右上角「上次选择」角标随语义消失而移除），故设置面板所见即所得；老用户 `omniterm_last_terminal_engine` 存档在首次读取时一次性继承，习惯的引擎不变。因宿主缺复用器而落到 pty 属环境限制而非用户意图，这类回落不写回设置，避免静默改掉 tmux 偏好（`frontend/src/utils/terminalEngine.ts`、`frontend/src/hooks/useTerminalEngine.ts`、`frontend/src/stores/appStore.ts`、`frontend/src/components/Settings/Settings.tsx`、`frontend/src/components/Sidebar/CreateSessionModal.tsx`、`frontend/src/components/FileManager/{FileManager,OpenTerminalDialog}.tsx`、`frontend/src/locales/{zh,en}/translation.json`）
- (2026-09-11 01:25) `[frontend]` `[backend]` `[api]` Sidebar 升级功能由一键自动重启改为手动重启按钮，并将重启时机选择权交给用户：一键升级完成后不再延时 3 秒自动杀进程重启，升级面板提供「立即重启」按钮；若用户关闭弹窗或未及时重启，在标题栏旧版本号旁常驻显示「REBOOT」徽标，点击可再次唤起面板手动触发重启；后端新增 `POST /system/restart` 端点，`GET /system/version` 返回 `pending_restart` 标志保证刷新或跨标签页时待重启态不丢失（`src/api/system.rs`、`frontend/src/api/client.ts`、`frontend/src/components/Sidebar/UpdateBadge.tsx`、`frontend/src/index.css`、`frontend/src/locales/{zh,en}/translation.json`）

### Fixed

- (2026-09-20 22:10) `[frontend]` 修正 ACP 用量徽章的费用符号：费用数值由 agent 随 `usage_update` 上报（含 `amount` + `currency`，OmniTerm 不做任何计算），但展示层恒在数值前拼 `$`——agent 报 CNY/JPY 等非美元时会把人民币/日元金额显示成美元，属错误信息。现按 agent 给的 ISO 4217 代码映射符号（`$ ¥ € £ ₩ ₹ ₽ ₺` 等常见币种，大小写不敏感），未覆盖的币种降级为「代码 + 数值」（如 `CHF 0.0450`），agent 未提供 currency 时只显示数值、不再臆造符号（`frontend/src/components/Chat/UsageIndicator.tsx`）
- (2026-09-19 09:45) `[frontend]` 移动端创建会话弹窗不再自动聚焦名称输入框：弹窗打开即聚焦会让手机立刻弹出软键盘、盖住下方的引擎卡片与选项（得先收起键盘才能选）；现仅在非移动端自动聚焦，移动端只预设默认值不夺焦点，桌面端交互不变（`frontend/src/components/Sidebar/CreateSessionModal.tsx`）
- (2026-09-18 00:45) `[frontend]` 修正 ACP 消息输出速度估算：独立计时并排除可识别的纯工具执行区间，保持工作时长包含工具；消息底部新增估算工具耗时，并行执行按并集、审批不重复扣除、重连重开观测窗口。
...
- (2026-09-08 22:50) `[backend]` 修复从 SSH 会话启动 OmniTerm 后，pty/tmux 终端里的 CLI（实测 agy/antigravity）被误判为 SSH 会话而要求重新登录并报错：后端经 SSH 登录 daemon 化时进程树继承 `SSH_CLIENT`/`SSH_CONNECTION`/`SSH_TTY`，派生出的本地终端会话也全带这三项——agy 见**任一** `SSH_*` 变量即改用文件 token 存储（日志 `Using file-based token storage because SSH session detected`；仅剩 SSH_CLIENT/SSH_TTY 也触发），读不到原生终端（无 SSH 变量）登录时写入 keyring 的 token。现 OmniTerm spawn 的所有本地进程（pty shell、tmux client、ACP 终端命令）一律移除这三个变量；tmux 新 session 的初始 pane 另经 `exec env -u ... $SHELL` 在命令源头剥离（SSH_CLIENT/SSH_TTY 不在 tmux 默认 `update-environment` 列表、仅清 client 环境会残留，实测确认），session env 由 set-environment 兜底后续新 pane——用户自己的 tmux 会话不受影响。端到端实测：真实 OmniTerm tmux 会话内 agy `keyringAuth: loaded token, expired=false` 免登录退出码 0（`src/engine/pty_io.rs`、`src/engine/pty/mod.rs`、`src/engine/tmux/{mod,control_mode,terminal_ws,watch_source}.rs`、`src/acp/terminal.rs`）
- (2026-09-07 20:40) `[backend]` 修复 npm 渠道一键升级「承诺自动重启却静默失败、日志零线索」（实测 `omniterm start -d -H 0.0.0.0` 局域网远程接入，v0.2.19 升 v0.2.20 后服务未换血、刷新仍旧版）：npm reify 升级时先把旧包目录 retire（rename）再删，Linux 上 unlink 运行中 exe 成功——exec 自重启捕获的目标路径随旧 inode 一起消失，exec ENOENT 旧进程继续服务；同时 daemon 继承的 `RUST_LOG` 含旧仓库 crate 名 directive（`omniterm_main`/`omniterm_server`）且 EnvFilter 无 catch-all，本 crate 全部日志被滤、失败 error 不可见。三层修复：`rust_log_covers_omniterm` 检测 RUST_LOG 未覆盖本 crate 时追加 `omniterm=info` 保底（零日志盲区）；自重启链诊断改 `eprintln` 直写 stderr 绕过 EnvFilter（daemon 模式 stderr 即日志文件），`relaunch()` 对 exec 目标加存在性预检把死路径转成明确报错；`current_exe_channel()` canonicalize 失败回退原始路径（死路径仍可判渠道，升级徽章与 restart_command 链路不再整个 500），`restart_command` 把含 `node_modules` 的 argv[0] 归一为 PATH 上的 shim `omniterm`（npm 渠道回显的原生二进制路径升级后必然失效，照抄会 `no such file`）。模式沉淀见 `docs/dev/debug-patterns/platform-protocol.md` 模式 9/10（`src/main.rs`、`src/api/system.rs`、`src/update.rs`）

## [0.2.20] - 2026-09-07

### Added

- (2026-09-04 23:03) `[frontend]` 创建会话弹窗的 ACP agent 选择对齐终端引擎的记忆体验：agent 此前每次打开都默认选列表第一个，常用非首个 agent 的用户要重复点选。现按「本次点选 > 上次成功创建的 agent（localStorage `omniterm_last_acp_agent`，仅创建成功才记）> 第一个」生效，上次用过的 agent 选项文本追加「上次选择」后缀；记忆 id 已被删除时回落第一个，ACP 创建不再顺带刷新引擎记忆（反之亦然，各记各的类别）（`frontend/src/components/Sidebar/CreateSessionModal.tsx`、`frontend/src/components/AgentPicker/AgentPicker.tsx`、`frontend/src/stores/appStore.ts`、`frontend/src/locales/{zh,en}/translation.json`）
- (2026-09-03 00:01) `[frontend]` ACP 会话流式期间显示实时工作计时：此前「已工作 X秒」只在 turn 定稿后一次性出现，跑一条长任务时气泡底部整段是空的，看不出 agent 已经干多久。现在 assistant 气泡出现的同一刻，底部元信息行（与 hover 动作栏同一行、同一右缘）开始跳「工作中 42秒」，定稿当场被后端结算值替换。它是**本地估算、只渲染**：不入库、不参与同步，后端 `work_ms` 仍是唯一真相源；等真人审批期间**冻住不跳**以贴齐 `work_ms = 墙钟 − 等真人` 口径（并发多个审批只批掉一个仍算挂着，对齐后端 `wait_depth`），文案「工作中」vs「已工作」、色板 `--text-muted` vs `--text-faint`、tooltip 三处标注估算性质。每秒跳动不进 React state（`ChatMessageView` 靠 memo 跳过重渲染，setState 会把整列每秒重画一遍），定时器直写 DOM，与底部乱码特效同手法且互不交叠；计时按 sessionId 分列（切会话不拆 WS，多路 turn 可并发），表有界 `MAX_TRACKED_TURNS=16` 最旧淘汰。停表路径穷举 `markDone`/`markError`/`markEnded`/`reset`（漏一条就留下一个永不落定的数字，`chatStore.test.ts` 逐条守住）。刷新/重连接回半程 turn 时锚点取在建行 `created_at`——该值是后端首次 flush 建行时刻、晚于 turn 起点，读数偏小属预期（宁可少报不虚报）。翻盘了原计划「流式期间不做计时中占位」的排除项，见 `docs/dev/plans/archive/2026-08-30-acp-work-time.md` E12（`frontend/src/utils/turnClock.ts`、`frontend/src/stores/chatStore.ts`、`frontend/src/components/Chat/ChatMessage.tsx`、`frontend/src/components/Chat/ChatView.tsx`、`frontend/src/locales/{zh,en}/translation.json`）
...
### Fixed
- (2026-09-06 17:49) `[backend]` `[frontend]` 修复 pty 会话多行粘贴在 agent TUI 输入框内被逐行当 Enter 提交：pty 是「双终端模拟器」架构，TUI 发的 bracketed paste 模式序列（`?2004h/l`）被 cell_frame 架构吞掉（raw 流只排干不转发），前端 xterm 的模式恒为关闭——桌面端 Ctrl+V 不加 `200~/201~` 包装裸发，多行文本里的换行被 TUI 逐行提交；移动端长按粘贴更是完全绕过 xterm 直发，连换行转换都没有（tmux 会话移动端粘贴同样受损）。现后端把模式真值放进所有 cell_frame（新增 `bracketed_paste` 字段，约 20 字节/帧），前端与 xterm 实际状态不一致时写回模式序列同步（幂等，会话切换 reset 后首帧自愈；被视口门控丢弃的帧同样消费）；移动端长按粘贴改走 xterm 原生 `paste()`，自带换行转换与按模式的包装，tmux 会话顺带受益（`src/engine/pty/frame.rs`、`src/engine/pty/vt.rs`、`frontend/src/hooks/useCellFrame.ts`、`frontend/src/hooks/useTerminal.ts`、`frontend/src/components/Terminal/Terminal.tsx`，方案见 `docs/dev/plans/2026-09-06-pty-bracketed-paste-relay.md`）
- (2026-09-05 16:47) `[backend]` `[frontend]` 修复 pty 会话「TUI 画在输入行上方、屏幕底部垫陈旧内容」的错位（与上条同症状家族的另一根因）：服务端 grid 与前端 xterm 是双模拟器镜像，行数靠 resize 消息同步，但连接初期容器布局未稳时（侧栏动画/字体加载），onResize 的 resize 消息会落在 WS open 之前被 readyState 门禁静默丢弃——此后两端行数永久分叉，cell_frame 只覆盖顶部 `height` 行，xterm 底部多余的行不被任何帧触碰、停留上次尺寸匹配时的旧画面。三层修复：WS open 时无条件补发当前尺寸（后端同尺寸幂等）；帧消费侧检测帧 `height/width` 与本端 `rows/cols` 不符即补发 resize，任何分叉约 1 帧内自愈（实测外部强缩后端到 20 行，浏览器 1 帧拉回 48）；回底校准请求（y=0）跳过指纹重定位——live 屏顶行与历史空行/提示符行同内容是常态，吸附会把回底帧顶成历史窗口使视图卡在 viewport 模式（指纹锚点计划「重复行误匹配视觉等价无害」的原判在此场景不成立，见计划文档勘误）。排查经 tmux capture-pane 参考对照排除 TUI 自身预留行行为与后端 grid，再以双端行数对比定罪（`src/engine/pty/vt.rs`、`frontend/src/hooks/useTerminal.ts`）
- (2026-09-05 00:20) `[frontend]` 修复 pty 终端跑 TUI 程序（codebuddy 等）时画面错位或冻结：TUI 画在输入行上方、底部残留 shell 回显，切换会话（补屏全帧）才恢复。根因是前端渲染队列（`useCellFrame`，上限 120 帧）在主线程卡顿/标签页隐藏期间积压超限时**清空全部积压帧且重同步请求被 1s 节流吞掉**——被清的帧里若有 full/overlay（TUI 启动的 alt-screen 清屏帧）或「擦除 shell 行」的 diff，后端 diff 基线已前进不会重发，错位/冻结永久保持。现超限时**保留积压中最后一个 full/overlay 帧作为自愈锚点**（full 自含完整屏幕状态：丢弃它之前的 diff 无损，保留它渲染即恢复，无需请求），仅当队列全是 diff（或 full 在队首无法瘦身）才清空；同时保证「**清空必有重同步在途**」不变式——节流窗口内的清空安排补发定时器，到点必发。实测 rAF 停摆 8 秒 + TUI 启动场景：修复前画面永久冻结在旧状态，修复后 TUI 正常显示且重同步全帧补齐丢失增量（`frontend/src/hooks/useCellFrame.ts`）
- (2026-09-03 12:40) `[backend]` `[frontend]` 修复 pty 终端上翻查看历史时内容缓慢滑动、顶部旧内容像被冻结：视口偏移 `y` 是「距实时屏底部的行数」，不是稳定标识——新输出会让同一个 y 指向更新的内容。原实现由前端拿 `history_size` 反推绝对行号补偿，而该值取自主线程收到的上一帧（最坏落后 33ms）再叠加 rAF 延迟，每次重拉都漂约 0.5 行（实测：20 行/秒输出下视图以 5.6 行/秒滑动，100 行/秒下 20 秒漂 109 行）；历史满 1000 行后 `history_size` 恒定、无法表达期间新增行数，视图更以 100% 输出速率滑动。现改为**按内容指纹锚定**：请求带窗口首行指纹，后端在持有 VT grid 的一侧于有界半径（512 行）内由近及远把窗口重定位到该行当前位置，回传实际 y 与新指纹；用户主动滚动不带指纹（按偏移定位），新输出重拉带指纹（保持锚点）。实测两种场景下锚定内容恒定，直到该行被 scrollback 回绕淘汰才开始滑动——与 xterm/wezterm 一致。指纹失配（内容被淘汰、被 TUI 重写、resize reflow）时退回按偏移定位，降级不冻结（`src/engine/pty/vt.rs`、`src/engine/pty/frame.rs`、`src/engine/pty/terminal_ws.rs`、`src/ws/terminal.rs`、`frontend/src/utils/viewportController.ts`、`frontend/src/hooks/useCellFrame.ts`、`frontend/src/hooks/useTerminal.ts`、`docs/dev/plans/2026-09-03-pty-viewport-fingerprint-anchor.md`）
- (2026-09-02 20:20) `[frontend]` 修复右下角提示无法复制报错信息：整条 toast 挂着 `onClick` 关闭，用户想选中报错文字时点一下就没了，且 4 秒定时在 `addToast` 里一次性排定、跑完必消失。现计时下沉到每条 toast：鼠标 hover 或键盘聚焦期间**暂停**倒计时、移开后接着走剩余时间；关闭只走行内 `✕` 按钮（补 `onClick` 与 `aria-label`，此前仅靠冒泡），正文 `select-text` 可自由选中，并按 Modal 的先例加 `overflow-wrap: anywhere`，避免长路径/URL 的报错画到框外。同屏 toast 上限 5 条、超出丢弃最旧的，报错风暴不再堆满屏幕（`frontend/src/components/Toast/Toast.tsx`、`frontend/src/stores/toastStore.ts`）
...
- (2026-08-30 16:10) `[backend]` `[frontend]` `[api]` ACP 会话工作时长计时：每条 assistant 回复的 hover 动作栏同行右端新增「已工作 2分钟42秒」（该行放不下时才另起一行，右缘始终贴合气泡右缘）。口径是 **agent 实际工作时长** —— turn 定稿时结算 `work_ms = wall_ms - wait_ms`，等人工审批的挂起时长单列 `wait_ms`、不计入工作（悬停该行可见），审批无人应答由 reaper 30 分钟兜底回收，时长不再虚增。`sessions` 表加 `work_ms/wait_ms/turn_count/last_turn_at` 增量列（写时结算，读会话列表零聚合成本，跨进程回收/重启不清零），`chat_messages` 加 `duration_ms/wait_ms`（NULL=未知，区别于 0）；`prompt_done` WS 帧新增 `duration` 字段，耗时在定稿那一刻即显示，不必等刷新读库。迁移前的历史行无数据 → 前端整行不渲染，不出现 `0秒`/`NaN`。单位字形由 `Intl` unit-narrow 按界面语言给出（中文「2分钟42秒」/英文「2m42s」），代码内不硬编码「分/秒」；时长只有后端一套真相，流式期间不做「计时中」占位（`migrations/20260830_add_work_time.sql`、`src/acp/turn_accumulator.rs`、`src/acp/client.rs`、`src/acp/reaper.rs`、`src/api/sessions.rs`、`src/ws/acp.rs`、`frontend/src/components/Chat/ChatMessage.tsx`、`frontend/src/index.css`、`frontend/src/utils/formatTime.ts`）
- (2026-08-29 23:19) `[frontend]` 桌面端切换/选中会话后自动聚焦终端：此前点击侧栏会话后 DOM 焦点仍留在侧栏行上，必须先点一下终端才能键入。现终端就绪与会话切换后自动调 `term.focus()`，覆盖侧栏点击、文件管理器「在此打开终端」、新建会话后自动激活、快捷键切会话四条激活路径。移动端不自动聚焦——聚焦 xterm 隐藏 textarea 会弹起软键盘遮住刚切过去的终端（`frontend/src/hooks/useTerminal.ts`、`frontend/src/components/Terminal/Terminal.tsx`）
...
- (2026-08-28 16:40) `[backend]` pty 终端历史滚动提速 4.5 倍（帧瘦身连带修掉的两个延迟源）：① diff 行指纹原为整行构造 `CellData`（40×100 屏即 4000 次 String 分配/帧），30fps 实时帧实测占 7.5 ms/帧、占住转发循环 23% 的时间，改为直接遍历 grid 计算指纹、零分配；② accept 出的 TCP 连接此前保持系统默认的 Nagle 开启，交互式小帧要等前一个包的 ACK，与对端 40 ms Delayed ACK 叠加使尾延迟涨到 ~50 ms，现对每个连接开 `TCP_NODELAY`。合计：请求→响应 p50 12.75 ms → 2.84 ms、p95 15.87 → 5.38 ms、max 15.93 → 6.07 ms，实时 diff 帧平均 3758 B → 409 B（`src/engine/pty/vt.rs`、`src/engine/pty/frame.rs`、`src/main.rs`）
...
- (2026-08-29 00:50) `[frontend]` Sidebar ACP 会话超 5 个自动折叠：单个 worktree 下 ACP 会话超过阈值时只露出最新 5 个，可见行与隐藏行之间放醒目的粗体「展开更多（+N）/收起」切换行，展开时隐藏行从切换行下方就地追加（不回插列表原位置，语义与点击处一致）；激活、等待用户决策或需注意力的会话豁免折叠始终露出，终端会话不参与折叠。展开状态为组件本地状态，刷新回到折叠（`frontend/src/components/Sidebar/ProjectCard.tsx`）
...
- (2026-08-27 23:38) `[frontend]` 修复切换终端会话时上一会话的 buffer 内容闪现在新会话里、且键盘输入打到旧会话：xterm 实例在会话切换间复用，旧 buffer 未清就渲染新会话；键盘处理器的 WS 闭包捕获的是挂载时的会话引用，切换后仍往旧连接写。现切换时清 buffer 并重建引用（`frontend/src/hooks/useTerminal.ts`）
- (2026-08-27 23:31) `[frontend]` 修复 pty 终端上翻回看历史时被全帧重绘冲回底部：全帧写入原先无条件清屏，用户滚到一半就被推回实时画面；差分帧也被误判走全屏路径。现用户滚动期间跳过全帧写入，且 `isFull` 判定按帧类型区分（`frontend/src/hooks/useCellFrame.ts`、`frontend/src/hooks/useTerminal.ts`）
...
- (2026-07-25) `[frontend]` ACP 聊天排队后续消息（queued follow-up）：agent 忙碌期输入框保持可编辑，按 Enter 不打断当前任务而是暂存到单槽队列，agent `prompt_done` 后自动发出。chatStore `queuedMessage` 字段（sessionStorage 镜像，`omniterm_chat_queue:{sid}`，F5 友好）+ `enqueueMessage` / `clearQueuedMessage` / `hydrateQueuedMessage` / `addUndeliveredMessage` actions；ChatInput 输入框上方加 `Next: <预览> ✕` chip，busy 时双按钮（`Cancel` 取消 in-flight + `Queue` 排队新消息），队列满时 Queue 按钮 disabled 强制先 ✕。断连时未发队列消息作为 `undelivered: true` user 消息写入内存流（不入库）留痕。drain 在 `useAcpChat` `prompt_done` 分支内联处理（避免 `useCallback` TDZ 与 ChatView 状态机同步问题），详见 `docs/architecture/adr/0001-acp-queue-drain-location.md`；domain glossary 见 `CONTEXT.md`（`frontend/src/stores/chatStore.ts`、`frontend/src/hooks/useAcpChat.ts`、`frontend/src/components/Chat/{ChatInput,ChatView,ChatMessage}.tsx`、`frontend/src/locales/{en,zh}/translation.json`、`docs/architecture/frontend.md`）
### Changed
