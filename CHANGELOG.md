...
- (2026-07-03) `[frontend]` Sidebar 顶部横幅 暗色模式色值一致化 — 发现两处不一致：**(1) `OmniTermLogo` 外框颜色 `#3A2E1F` 硬编码亮色 wood-shadow**，暗色模式 title bar border 变为 `#090A0D` 后，logo 外框仍是亮棕色，违背「CRT 嵌入标题栏底边」的设计意图；**(2) `logo-version` 暗色被改成 `#8B7755` muted brown** 而 `title-bar-path` 保持 `#FFCB6B` 金色，两个同角色 subtitle 颜色不一致。修复：新增 CSS token `--logo-frame`（亮 `#3A2E1F` / 暗 `#090A0D`）给 logo SVG 外框使用；删除 `.dark .logo-version` 覆盖；`logo-version` 与 `title-bar-path` 统一改用 `var(--gold-light, #FFCB6B)` 金色 token；`docs/ui-style-guide.md` §3.1 记录 `--logo-frame` 的设计动机与 SVG `style={{ fill: 'var(...)' }}` 使用方式（`frontend/src/components/PixelUI/OmniTermLogo.tsx`、`frontend/src/index.css`、`docs/ui-style-guide.md`）
