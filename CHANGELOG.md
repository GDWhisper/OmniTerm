...
- (2026-09-08 21:00) `[backend]` 修复从 SSH 会话启动 OmniTerm 后，pty/tmux 终端里的 CLI（实测 agy/antigravity）被误判为 SSH 会话而要求重新登录并报错：后端经 SSH 登录 daemon 化时进程树继承 `SSH_CLIENT`/`SSH_CONNECTION`/`SSH_TTY`，派生出的本地终端会话也全带这三项——agy 见 `SSH_CONNECTION` 即改用文件 token 存储（日志 `Using file-based token storage because SSH session detected`），读不到原生终端（无 SSH 变量）登录时写入 keyring 的 token。现 OmniTerm spawn 的所有本地进程（pty shell、tmux client、ACP 终端命令）一律移除这三个变量；tmux 侧另对新建 session 显式 unset 兜底（`SSH_CLIENT`/`SSH_TTY` 不在 tmux 默认 `update-environment` 列表，实测确认），用户自己的 tmux 会话不受影响（`src/engine/pty_io.rs`、`src/engine/pty/mod.rs`、`src/engine/tmux/{mod,control_mode,terminal_ws,watch_source}.rs`、`src/acp/terminal.rs`）
