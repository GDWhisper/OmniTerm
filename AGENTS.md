...
| `docs/dev/plans/2026-08-11-chat-bubble-actions.md` | 修改 `ChatMessage.tsx` 的消息动作、块级操作、剪贴板相关代码（`utils/clipboard.ts` / `messageActions.ts` / `MessageActionBar.tsx` / `useLongPress.ts` / `chatStore.pendingInsert`）前**必读**（动作注册表 D2、长按触发区域 D3、引用通道 D7、删除/截断排除 D6） | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
