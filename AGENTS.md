...
| `docs/dev/plans/2026-08-30-acp-work-time.md` | 修改 `turn_accumulator.rs`（`WriterCmd` / `finalize_turn` 记账）、`chat_persistence.rs`（`finalize_message` / `ChatMessageRow`）、`client.rs`（审批 pause / shutdown 收尾）、`sessions` 时长列、`ChatMessage` 耗时任一项前**必读** | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
