...
| `docs/dev/plans/2026-09-03-pty-viewport-fingerprint-anchor.md` | 修改 pty 历史视口锚定相关代码前**必读**——「距底偏移 y 不是稳定标识」这条结论是两轮排查的产出，不看方案容易退回按位置换算的老路；其 D1-D5 指纹重定位机制已被 09-12 有状态锚取代（文末勘误），仅作历史决策与结论来源 | 勘误已闭环（2026-09-12 机制被取代）；如再翻盘按惯例就地加「勘误」块 |
...
| `docs/dev/plans/2026-09-10-sidebar-session-context-menu.md` | 修改 Sidebar 会话行（`ProjectCard.tsx` 的 `SessionRow`、`SessionContextMenu.tsx`、`BatchActionBar.tsx`、`BatchSessionDialog.tsx`：右键/长按菜单、批量选择模式、归档/释放/删除跳过语义）前**必读**——D1-D7 决策（含长按补发 click 抑制、释放池幂等依据）在此 | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
