...
| `docs/dev/plans/2026-09-21-acp-agent-connection-cpu-spin.md` | 修改 `src/acp/client.rs` 的 `shutdown`/`disconnect`/连接任务生命周期、`src/api/sessions.rs` 的 release/archive 路径，或排查「恢复 ACP 会话后后端 CPU 飙高」前**必读**——「连接 poll 卡死时优雅关闭路径永远走不到、release 注释声称杀进程实际没有」的结构性缺陷与 D1-D5 决策（含交接说明）在此；完整证据链先读 `docs/dev/diagnostics/2026-09-21-omniterm-cpu-spike.md` | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
| `docs/dev/plans/2026-09-22-tmux-server-shutdown-hang.md` | 修改 `src/engine/tmux/control_mode.rs`（`ControlModeClient` spawn/`stop()`/`Drop`）、`src/engine/tmux/terminal_ws.rs`、`src/engine/tmux/engine.rs`/`mod.rs` 的 tmux 子进程生命周期，或排查「tmux 命令报 `server exited unexpectedly` / tmux server 假死 / `tmux -C` 孤儿客户端堆积」前**必读**——「SIGTERM（来源未确证）后 tmux 关闭流程被停止 drain 的孤儿 control 客户端无限期冻结（`control_all_done` 无超时）+ omniterm 崩溃路径不收尸约 4.5 周积 28 个孤儿」双缺陷根因与 PDEATHSIG/启动对账/聋 server 自愈方案（含 ADR D1-D4）在此 | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
