...
| `docs/dev/plans/archive/2026-08-18-permission-recycle-notice.md` | 修改 `src/acp/reaper.rs`（回收分支）、`src/acp/client.rs`（system 通知通道）、`src/ws/acp.rs`（system_message 帧）、`chat_messages.role` 语义任一项前**必读** | 行为变更、勘误时更新 |
...
| `docs/dev/plans/archive/2026-08-30-acp-work-time.md` | 修改 `turn_accumulator.rs`（`WriterCmd` / `finalize_turn` 记账）、`chat_persistence.rs`（`finalize_message` / `ChatMessageRow`）、`client.rs`（审批 pause / shutdown 收尾）、`sessions` 时长列、`ChatMessage` 耗时、`utils/turnClock.ts`（流式实时计时 / 工具时长 / tps 估算）、`useAcpChat` 计时接线任一项前**必读**——速度与工具时长的观测口径、并行/审批边界、重连重开观测窗见 E12–E14 | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
| `docs/dev/plans/archive/2026-09-03-pty-viewport-fingerprint-anchor.md` | 修改 pty 历史视口锚定相关代码前**必读**——「距底偏移 y 不是稳定标识」这条结论是两轮排查的产出，不看方案容易退回按位置换算的老路；其 D1-D5 指纹重定位机制已被 09-12 有状态锚取代（文末勘误），仅作历史决策与结论来源 | 勘误已闭环（2026-09-12 机制被取代）；如再翻盘按惯例就地加「勘误」块 |
| `docs/dev/plans/archive/2026-09-08-pty-incremental-sync-hardening.md` | 修改 cell_frame 增量同步链路（`src/engine/pty/vt.rs` 的 `encode_cell_frame`/帧序号、`terminal_ws.rs` 转发循环、`frame.rs` 的 `CellFrame.seq`、`frontend/src/hooks/useCellFrame.ts` 队列、`useTerminal.ts` 状态行写入）前**必读**——「无校准的增量镜像必不自愈」是六轮修后仍复发的结论，止血方案 A（周期全帧+seq）+C（状态行 resync）在此 | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
| `docs/dev/plans/archive/2026-09-12-pty-viewport-stateful-anchor.md` | 修改 pty 历史视口锚定相关代码（`src/engine/pty/vt.rs` 的 `encode_viewport_frame`/滚移检测、`src/ws/terminal.rs` 的 `ViewportRequest` 协议、`frontend/src/utils/viewportController.ts` 的请求意图链路）前**必读**——「指纹重定位在周期性内容上必棘轮/滑移（p<2k），重定位型锚点必须有位置记忆」的实证结论与修复设计（后端有状态锚+滚移检测）在此，取代 09-03 指纹机制 | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
| `docs/dev/plans/archive/2026-09-10-chat-file-attachments.md` | 修改聊天输入框附件链路（`ChatInput` 的 + 按钮/隐藏 input/文件 chip、`ChatAttachDrawer.tsx`、`fileAttachment.ts`、WS `prompt.files` 帧、`build_prompt_blocks`）前**必读**——「文件走 base64 blob 内联、落库只存元数据、能力缺失即拒绝」三条决策及翻盘条件在此 | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
...
| `docs/dev/plans/2026-09-19-acp-failure-visibility.md` | 修改 turn 结束语义（`src/ws/acp.rs` 的 `dispatch_prompt`、`src/acp/turn_accumulator.rs` 定稿状态、`src/acp/chat_persistence.rs` 的 `sync_messages` 匹配、`frontend/src/hooks/useAcpChat.ts` 的 `prompt_done`/`replay_end` 分支）前**必读**——「协议合法值 ≠ 成功语义：`end_turn` 之外的非正常终态必须留痕」「手动恢复重放绕过 hydrate 门控会产生重复行」两条实证结论与 D1-D4 决策在此 | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
| `docs/dev/plans/archive/2026-09-21-acp-agent-connection-cpu-spin.md` | 修改 `src/acp/client.rs` 的 `shutdown`/`disconnect`/连接任务生命周期、`src/api/sessions.rs` 的 release/archive 路径，或排查「恢复 ACP 会话后后端 CPU 飙高」前**必读**——「连接 poll 卡死时优雅关闭路径永远走不到、release 注释声称杀进程实际没有」的结构性缺陷与 D1-D5 决策（含交接说明）在此；完整证据链先读 `docs/dev/diagnostics/2026-09-21-omniterm-cpu-spike.md` | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
| `docs/dev/plans/archive/2026-09-22-tmux-server-shutdown-hang.md` | 修改 `src/engine/tmux/control_mode.rs`（`ControlModeClient` spawn/`stop()`/`Drop`）、`src/engine/tmux/terminal_ws.rs`、`src/engine/tmux/engine.rs`/`mod.rs` 的 tmux 子进程生命周期，或排查「tmux 命令报 `server exited unexpectedly` / tmux server 假死 / `tmux -C` 孤儿客户端堆积」前**必读**——「SIGTERM（来源未确证）后 tmux 关闭流程被停止 drain 的孤儿 control 客户端无限期冻结（`control_all_done` 无超时）+ omniterm 崩溃路径不收尸约 4.5 周积 28 个孤儿」双缺陷根因与 PDEATHSIG/启动对账/聋 server 自愈方案（含 ADR D1-D4）在此 | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
...
| `docs/dev/plans/archive/2026-09-21-permission-timeout-modes.md` | 修改权限请求超时行为/设置（`src/acp/reaper.rs` 超时三分支、`src/acp/permission.rs` 的 `pick_auto_option`/请求摘要、`src/api/settings.rs` 的 permission-timeout 路由、`src/main.rs` 的 `PermissionTimeoutConfig`、`Settings.tsx` 的 `PermissionTimeoutSection`、system 消息 detail 渲染）任一项前**必读**——「abort/auto/wait 三模式语义、auto 选项优先级 allow_always→allow_once（用户拍板）、告知载荷 label+detail、wait 跳过 prompt-stale」的决策与已知边界在此 | Phase 推进、决策翻盘（尤其 D2 选择规则）、实施偏差（就地加「勘误」块） |
