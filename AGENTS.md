...
| `docs/dev/plans/2026-09-22-tmux-server-shutdown-hang.md` | 修改 `src/engine/tmux/control_mode.rs`（`ControlModeClient` spawn/`stop()`/`Drop`）、`src/engine/tmux/terminal_ws.rs`、`src/engine/tmux/engine.rs`/`mod.rs` 的 tmux 子进程生命周期，或排查「tmux 命令报 `server exited unexpectedly` / tmux server 假死 / `tmux -C` 孤儿客户端堆积」前**必读**——「外部 SIGTERM 后 tmux 关闭流程被停止 drain 的孤儿 control 客户端无限期冻结（`control_all_done` 无超时）+ omniterm 崩溃路径不收尸六周积 28 个孤儿」双缺陷根因与 PDEATHSIG/启动对账/聋 server 自愈方案在此 | Phase 推进、决策翻盘、实施偏差（就地加「勘误」块） |
