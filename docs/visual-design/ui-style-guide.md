...
| `.font-pixel` | `'Pixelify Sans', 'VT323', 'Press Start 2P', monospace` | Titles, buttons, status labels, short display text |
...
| `--pixel-font` | `'Pixelify Sans', 'VT323', 'Press Start 2P', monospace` | `PIXEL_FONT` |
...
| `body.pixel-font-on` | always | `.font-logo` and `.font-pixel` use Press Start 2P / Pixelify Sans |
...
| 元素 | 规格 |
|------|------|
| Tab 字体 | `.font-pixel` (Pixelify Sans) 14px，letter-spacing 1.5px，**UPPERCASE** |
...
- **Tab 文字**：英文 UPPERCASE（Pixelify Sans/VT323 不支持中文，pixel 风统一）
