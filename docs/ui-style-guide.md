...
| Token | Hex | RGB | 用途 |
|---|---|---|---|
| `bg-base` | `#0a0a0f` | `10, 10, 15` | 所有面板背景、空状态、drag bar 底色 |
| `bg-elevated` | `#111827` | `17, 24, 39` | 浮动层：hover 背景、context menu、modal、按钮 hover |
...
| Token | Hex | 用途 |
|---|---|---|
| `border-subtle` | `#1e293b` | 主要分隔线：sidebar 右边、toolbar 下边、input 默认边、drag bar 上下边 |
| `border-strong` | `#334155` | 浮动层边线：context menu、modal、toggle-view dropdown、滚动条 thumb |
...
| Token | Hex | 用途 |
|---|---|---|
| `text-primary` | `#e2e8f0` | 主要内容：文件名、工作区名、modal 标题、正文 |
| `text-secondary` | `#cbd5e1` | 次要内容：folder 名、breadcrumb、context menu 项 |
| `text-muted` | `#94a3b8` | 辅助：列头 (名称/修改时间/大小)、toolbar icon、空状态主文案、`文件信息` |
| `text-faint` | `#64748b` | 占位/禁用：drag bar 静息指示条、空状态次文案、禁用按钮 |
| `text-dim` | `#475569` | 最弱：空状态底部提示 ("使用左侧边栏开始")、禁用且半透明 |

### 2.4 强调色 (Accent)

| Token | Hex | 用途 |
|---|---|---|
| `accent-violet` | `#a78bfa` | **主强调**：hover 文本、选中背景叠加、drag bar hover 指示条、按钮 primary 背景、链接、进度条、focus ring |
| `accent-violet-bright` | `#c4b5fd` | 次级 violet：按钮 primary hover、更亮的 hover 文本 |
...
| Token | Hex | 用途 |
|---|---|---|
| `danger` | `#ef4444` | 删除、危险操作按钮 |
| `danger-12` | `rgba(239, 68, 68, 0.12)` | 危险按钮 hover 背景 |
| `danger-40` | `rgba(239, 68, 68, 0.4)` | 危险按钮默认边线 |
...
```css
track:      #0a0a0f      /* bg-base */
thumb:      #334155      /* border-strong */
thumb hover:#a78bfa      /* accent-violet */
width:      8px
```
...
- 空状态（未选中 session）：深黑底 + 居中 violet emoji + 中文提示文案
