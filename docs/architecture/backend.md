...
GET  /api/v1/system/version           # 版本检查（进程内缓存 GitHub latest，成功 1h/失败 5min；附 container 标记）
POST /api/v1/system/update            # 一键升级（github_release 自替换 / npm 代跑；cargo 返回 400；Unix 成功后备自动重启）
