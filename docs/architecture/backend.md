...
- **前端只呈现、不自算**：`prompt_done` 帧带 `duration{work_ms, wait_ms}`（取值方式与同帧 `row_id` 一致，见「重连续接协议」），耗时在定稿那一刻即显示；刷新后走 `GET /sessions/{id}/messages` 读库。**唯一呈现位**是 assistant 的 hover 动作栏所在行的右端「已工作 2分钟42秒」（`frontend/src/components/Chat/ChatMessage.tsx`，行放不下才由 flex-wrap 另起一行），等待时长挂该行 tooltip；该行右缘由 layout effect 实测最后一个正文块的宽度（气泡按内容收缩，CSS 表达不了跨兄弟节点右对齐），故同行与换行两种形态下都不越过气泡右缘。单位字形由 `Intl` unit-narrow 按界面语言给（不在代码里硬编码「分/秒」），未知 → 整行不渲染；**0 与亚秒分档**（0 = 该活动没发生，如从未等过审批；亚秒 = 发生了但不足一秒，收成 `<1秒` 而非 `0秒`）。
...
- **自动重启（Unix）**：`auto_restart: true` 时后端调度延迟任务（`RELAUNCH_DELAY`=3s，先让响应 flush 回前端）→ 回收 ACP 子进程（复用 `AcpSupervisor::shutdown_all`，包 `RELAUNCH_SHUTDOWN_TIMEOUT`=15s：回收依赖子进程退出确认，任一环节挂起会让 exec 永远执行不到且无日志，超时则记 warn 后照常 exec，残留子进程过继给 init）→ `update::relaunch()` 用 `execv` 替换为新二进制。**exec 目标必须是替换前 `current_exe_channel()` 捕获的规范化路径**（rename 覆盖后该路径指向新二进制）——事后重新解析 `current_exe()` 在 Linux 上会拿到带 ` (deleted)` 的失效路径、exec ENOENT 静默不重启（macOS 巧合可用，见 `docs/dev/debug-patterns/platform-protocol.md` 模式 10）。**PID 不变**：pid 文件、监听地址、daemon 身份零迁移；listen socket 由 tokio/mio 以 CLOEXEC 创建、exec 时内核自动关闭（无 `Address already in use`）；daemon 模式 log fd 非 CLOEXEC 被保留，新进程日志继续落同一文件。exec 参数剥离 `-d/--daemonize`（已在 daemon 环境，避免二次 double-fork 漂移 PID）。**exec 失败时旧进程继续服务**（只记 error 日志）。前端重启监测按**版本比对**确认：`/api/v1/health` 返回 `version` 字段，UpdateBadge 每秒轮询，`version` 等于升级响应返回的 `version` 即整页刷新——不要求捕捉断连瞬间，远程接入（SSH 隧道拆线、后台标签节流）错过断连窗口也能确认切换；升级来源是 health 无 `version` 字段的旧实现时回退「断连→恢复」状态机；60s 未确认（如 exec 失败）显示「服务尚未切换到新版本」的手动重启提示，不再承诺自动刷新。Windows 无 exec 等价物（`stop` 亦不支持），返回 `auto_restart: false`，维持手动重启提示（渲染 `restart_command`）。tmux server 为独立外部进程，重启 omniterm 不丢任何终端会话。
