...
> `docs/dev/plans/archive/2026-09-12-pty-viewport-stateful-anchor.md`）。饱和期（hs 恒
...
背景事故与修复计划：`docs/dev/plans/archive/2026-09-22-tmux-server-shutdown-hang.md`（实施偏差见其「实施勘误」章）。落点约束：P0-1/P0-2/P1-3 按「致命 bug 修复」豁免进 `src/engine/tmux/`；探测/分类/自愈/监控落引擎无关的 `src/health/`（ADR D4，冻结边界不解冻）。
