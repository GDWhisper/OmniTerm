...
- **自动重启（Unix）**：`auto_restart: true` 时后端调度延迟任务（`RELAUNCH_DELAY`=3s，先让响应 flush 回前端）→ 回收 ACP 子进程（复用 `AcpSupervisor::shutdown_all`）→ `update::relaunch()` 用 `execv` 替换为新二进制。**PID 不变**：pid 文件、监听地址、daemon 身份零迁移；listen socket 由 tokio/mio 以 CLOEXEC 创建、exec 时内核自动关闭（无 `Address already in use`）；daemon 模式 log fd 非 CLOEXEC 被保留，新进程日志继续落同一文件。exec 参数剥离 `-d/--daemonize`（已在 daemon 环境，避免二次 double-fork 漂移 PID）。**exec 失败时旧进程继续服务**（只记 error 日志），前端倒计时 60s 兜底后显示手动重启提示。Windows 无 exec 等价物（`stop` 亦不支持），返回 `auto_restart: false`，维持手动 `omniterm stop && omniterm start` 提示。tmux server 为独立外部进程，重启 omniterm 不丢任何终端会话。
