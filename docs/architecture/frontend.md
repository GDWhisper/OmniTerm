...
│   ├── en/translation.json
│   └── zh/translation.json
├── utils/               # 共享纯函数（path.ts, fonts.ts, agentAggregate.ts 会话组状态聚合 blocked>done>working——tmux agent_state 与 ACP chatStore 派生状态在此归一, imageAttachment.ts 聊天图片附件处理——mime 白名单/canvas 降采样/5MB 硬限, atReference.ts 聊天 @ 文件引用 token 检测/替换——与后端 extract_at_paths 语义对齐, touchScroll.ts 移动端终端触摸滚动桥（纵向 drag→合成 wheel）, viewportController.ts pty 历史视口状态机（方案 C：live/viewport 模式、wheel 接管、rAF 合并、alt-screen 互斥；2026-09-12 有状态锚——请求意图经 `refresh` 位显式区分（滚动 false / 输出重拉 true），锚的位置记忆驻后端 `VtState`，前端不做任何锚定换算，仅做响应 y 权威同步）, swipe.ts 移动端滑动切 tab 手势判定, haptics.ts 触觉反馈, sessionNav.ts 会话循环切换, clipboard.ts 统一剪贴板复制（async API + textarea 兜底，D1）, chatDraft.ts 会话草稿存取（sessionStorage，从 ChatInput 提取，D7）, messageText.ts extractMessageText 消息正文提取（复制/引用共用）, messageMarkdown.ts 单条消息导出 Markdown（D5）, proxyUrl.ts rewriteLocalUrl 本机 localhost 链接→/proxy/{port}/ 重写（端口转发代理 P3）, turnClock.ts 在建 turn 实时计时表（按 sessionId 分列、审批挂起冻表、`MAX_TRACKED_TURNS` 有界；刻意不进 zustand——每秒跳动的读数直写 DOM，渲染门控只依赖已有的 `sending`，见 `plans/archive/2026-08-30-acp-work-time.md` E12；本 turn 输出字符计数与 tps 估算同表扩展：`addOutputChars`/`turnTps`（流式实时）与 `finalTps`（定稿快照，0 输出清旧值防错配新消息行））, terminalEngine.ts 终端引擎偏好单一真源（默认 tmux、存档白名单解析、旧「上次使用」键一次性继承、`resolveTerminalEngine` 把缺复用器的回落收敛到一处）, …）
└── components/
...
- **pty 路径**：零注入字节——滚轮/翻页/退出滚动全部经 `utils/viewportController.ts`
  请求后端历史窗口帧（方案 C，D1-D6；2026-09-12 起锚定为后端有状态锚：滚动
  发 `refresh=false`、输出触发的重拉发 `refresh=true`，位置记忆驻后端
  `VtState`，前端零位置推算；`attachCustomWheelEventHandler` 接管
  wheel，alt-screen / 鼠标协议激活时交还 xterm 默认路径；开关
...
- **配置快照注入（已结束会话置灰配置栏）**：同一 `GET /messages` 响应还带 `configOptions`（后端持久化的最后已知配置快照，见 backend.md「配置快照」）+ `agentLive`（supervisor 存活位）。`ChatView` hydrate effect 在放行 `preHydrateBuffer` **之前**经 `setConfigSnapshot(sid, parseConfigOptions(data.configOptions), data.agentLive !== true)` 注入——已结束会话配置栏以快照置灰只读展示（样式同活跃态、整体 opacity 0.5、下拉 disabled；`set_config_option` 对无活 client 是静默 no-op，不禁用会无声失败），活会话刷新也注入值以消除配置栏空窗但不置灰。解析与 live 帧共用 `parseConfigOptions`（`useAcpChat` 导出，§7 单一真源）；解除只读的唯一路径是后续 live/replay 配置帧经 `applyTopLevelActions` 覆盖（自动置 `configReadOnly: false`），store 无绕过它的独立配置 setter（原零调用的 `setConfigOptions` action 已删）。
...
- **贴底跟随（`autoStick`）**：贴底态从滚动几何推导（距底 `<CHAT_STICK_THRESHOLD_PX` 视为贴底），`handleScroll` 是唯一解除点（用户上翻超阈值即解除并显示「回到底部」；点按钮 / 手滚回底即恢复）。**把底缘推远的不只有滚动**，故重钉是两条路径，改动时缺一不可：① 滚动内容高度的**全部**来源（`messages`、`sending` 思考指示、`replaying`、`terminalEvents`、`loadingHistory`）进贴底 effect 的 deps——往里新增条件渲染必须同步登记，否则内容长高而 deps 不动 = 无事件静默失跟；② 容器尺寸变化（键盘收放、todo 看板/权限条/输入区长高、拖面板宽度）经 `ResizeObserver` 重钉。**② 不可省**：浏览器只在滚动容器可见时才替我们保住底缘（滚动锚定），而移动端三面板同挂在一条 300% strip 上，聊天面板离屏时收缩实测底缘直接掉下去且全程不发 scroll 事件，`autoStick` 仍是 true → 切回 sidebar/files 即「没追底」且回底按钮也不显示。两条路径都以 `autoStick` 为门控，用户已上翻时一律不干预（阅读位置优先）；`autoStickRef` 与 state 同写入口，供 RO 回调读当前态而不重建 observer。
