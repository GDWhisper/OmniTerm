...
| 终端与 PTY | `debug-patterns/terminal-pty.md` | MasterWriter Drop 副作用、字节能到达≠语义能到达、跨进程分隔符选型、平替实现边缘语法、FitAddon 桌面像素常量、escape-time 时序矩阵、增量绘制 diff 流不可字节尾回放（字节流≠屏幕状态）、轮询推送的时延下界、diff 流不可 latest-wins 聚合、双终端模拟器重复应答（迟到应答透传回显）、拉模型视口需刷新触发源与逃生口（相对偏移在增长序列上不稳定）、TUI 程序≠用 alt-screen（前提须抓字节实测）、回放的历史是字节快照须补尾部语义边界（半行/UTF-8/未闭合转义序列；症状逐次累加=残留被复用） |
