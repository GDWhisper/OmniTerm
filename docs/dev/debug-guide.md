...
| 构建与协议 | `debug-patterns/platform-protocol.md` | 构建期字节契约（.gitattributes/checksum）、批量枚举 per-item 容错、三态布尔序列化、wire-format 抓帧、热路径 spawn 成本、渠道字节差异探针、Windows spawn 裸命令名（PATHEXT）、warn 被读成失败、通用名 env 被进程树继承劫持、自替换后 current_exe 失效（/proc/self/exe） |
