...
| 通用诊断手法 | `debug-patterns/investigation.md` | 诊断三阶段、分层二分+字节级证据、先写复现测试再定罪、验证脚本先自证、实物验证>代码推测、假设先行验证滞后、tracing directive 陷阱、宿主只见终态（病因在对端实现自己的日志里，协议合法值≠成功语义；准备期失败不留入参） |
