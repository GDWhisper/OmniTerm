...
| 终端与 PTY | `debug-patterns/terminal-pty.md` | MasterWriter Drop 副作用、字节能到达≠语义能到达、跨进程分隔符选型、平替实现边缘语法、FitAddon 桌面像素常量、escape-time 时序矩阵、增量绘制 diff 流不可字节尾回放（字节流≠屏幕状态）、轮询推送的时延下界 |
