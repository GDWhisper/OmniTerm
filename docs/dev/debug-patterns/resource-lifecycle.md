...
## 模式 10：父死不杀子——优雅清理只覆盖优雅路径，崩溃路径要内核级兜底

**资源-父殇兜底**：Linux 父进程死亡**不会**杀死子进程（无 PDEATHSIG 语义），`Drop`/`stop()`/信号处理器里的清理**只覆盖优雅退出**。panic-abort、SIGKILL、OOM-kill 面前，应用层清理代码等于不存在。凡是「父进程持有子进程、子进程又持有跨进程资源（连接/锁/会话）」的结构，崩溃路径必须靠内核级机制兜底：`prctl(PR_SET_PDEATHSIG)`，或在消费方（对端组件）做对账清理。只写优雅路径的清理 = 崩溃一次留一批，**按时间积分**——诊断时「同类残留物随运行时长单调增长」就是这条的签名。

**资源-外部组件被拖死**：孤儿子进程的危害不只是泄漏自己，更在于**它持有的对端资源会把对方拖死**：本案例 28 个孤儿 `tmux -C` 客户端各自持有 tmux server 的连接且停止 drain，导致 tmux 收到 SIGTERM 后关闭流程被无限期冻结（对端无超时），server 进入「进程活着但对所有新客户端立即断连」的半死态 18 分钟。判定链：对端假死 → 查它还持有多少来自死父进程的连接 → 残留物数量与崩溃次数正相关。**只收割僵尸（`waitpid`）不解决父殇留子**——前者收拾已退出的，后者防止父崩后继续活的，两者互补。

**诊断弯路**：「tmux 挂了」的第一反应是查 tmux 崩溃（dmesg/core/OOM 全部落空），或怀疑管理器误发信号（翻遍源码无果）。正确切入是**先分清「进程死了」与「进程半死」**：新连接被 ECONNREFUSED 才是前者；连上后立即 EOF 说明对端事件循环还活着、只是拒绝服务——这时去读对端源码里「什么条件下 accept 后立即 close」，直接命中退出流程卡死，再顺藤摸到卡在哪个退出条件上。

**适用**：任何 spawn 子进程且子进程持有跨进程/网络资源的代码（终端会话、SSH/ACP 连接、代理）；改子进程清理路径前，先问「父进程被 SIGKILL 时这条路径还在吗」。

**案例证据**：
- 2026-09-22 tmux server 假死事故：omniterm 多实例崩塌留下 28 个 PPID=1 的 `tmux -C` 孤儿（约 4.5 周积累，挂 22 个旧 session），tmux 3.4 关闭时 `control_all_done()` 无超时 → 一个 SIGTERM 后 server 聋了 18 分钟。完整证据链与修复方案见 `docs/dev/plans/archive/2026-09-22-tmux-server-shutdown-hang.md`。
- 同族对照：commit `344750f`（tmux 控制连接子进程退出后留僵尸）只加收割不防父殇；`docs/dev/plans/archive/2026-09-21-acp-agent-connection-cpu-spin.md` 的「优雅关闭走不到」是同一病根在 ACP 侧的投影。
