...
**适用**：任何既在开发环境里跑、又对外发布可执行文件的项目；症状是「同一份正式版在 A 终端能起、在 B 终端起不来」。**弯路**：报错是 `Address already in use`，第一反应是查端口占用/僵尸进程/PID 文件，实际端口值本身来自继承的环境——先 `printenv | grep -E '<全部候选变量名>'` 打印**用户实际 shell** 的环境，再看代码优先级链。
