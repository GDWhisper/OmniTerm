...
- [ ] 前端单测：replay 先于 hydrate 到达时不 INSERT 幽灵行（mock WS 帧序）
- [ ] 前端单测：hydrate 落定后 RAW 行被带 id 回写成 cooked，消息条数不增加
- [ ] 后端单测：`sync_messages` 带 id 路径对 RAW 行 UPDATE 不 INSERT（已有）；补"无 id 路径 text 不一致时不 INSERT 而是跳过或告警"？——**此改动会改变现有行为，需与维护者确认**（无 id 匹配失败目前 INSERT 是"重放重建消息落库"的合理语义，方案 A 已使该路径不再被幽灵行触发，故倾向**不改后端匹配语义**，只在测试里固化行为）
...
- 修复实施后：`CHANGELOG.md` Fixed 条目；`docs/architecture/backend.md`（replay 门控语义、hydrate 收敛）；本文件状态改「已实施」或移入 archive
- `docs/dev/debug-patterns/`：幽灵行的「replay 帧不受 hydrate 门控 + 无 id 文本匹配失败 → INSERT 重复行」值得提炼为模式（与 Phase 0 的「匹配键不唯一导致批量误更新」同族：**匹配键选错/匹配路径失控等于没有约束**）
- AGENTS.md 文档索引：登记本文件触发条件
