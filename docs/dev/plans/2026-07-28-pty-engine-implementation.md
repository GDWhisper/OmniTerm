...
> 状态：设计稿（2026-07-28，同日方向修订 v2；2026-08-09 盘点勘误 v3；2026-08-13 D8 选型翻盘 v5；**2026-08-20 方向修订 v6——长期双引擎共存**）
...
> **勘误（2026-08-20 v6，方向修订）**：产品决策再修订——**pty 与 tmux 长期共存**，不再是"过渡期共存 → 未来摘除"：创建会话时用户可选 pty / tmux，tmux 引擎作为维护态选项长期保留（冻结纪律不变：只修致命 bug 不加功能）。相应地：Phase 4 由「切换默认 pty」改为「前端双引擎可选」；**原 Phase 5（摘除 tmux）整节作废**；D6/D9/D10 中一切「摘除时一并处理」的措辞改为「冻结边界长期保留」。
...
- **tmux 专属**：`control_mode.rs`（403 行只做"最近 2s 有无输出"）、`mod.rs`（tmux 子进程命令门面）——随 TmuxEngine 冻结，长期保留于维护态（v6：不再规划摘除）。
...
### D6（v2 修订，v6 定案长期保留）：external 会话发现/收养 = tmux 专属能力
- **决策**：`/sessions/external`、`/sessions/adopt`、`/ws/terminal/external/{name}`、Sidebar 外部会话区块**长期保留于维护态、不再迭代**，实现收进 TmuxEngine 边界内（冻结纪律：只修致命 bug）。
- **理由**：功能现成可用；它本质依赖外部 tmux server，PtyEngine 无对应物，不做抽象化。v6 确定双引擎长期共存后无摘除时间表，该能力随之长期保留。
- ~~初版决策：立即全链路删除~~（v2 作废）。~~摘除 tmux 时一并删除~~（v6 作废）。
...
### D9（v2 重写，v6 修订为长期共存）：引入 `SessionEngine` 抽象，tmux 为冻结后端
- **决策**：定义 `SessionEngine` trait（方向规划 D3 提为 P0），能力面 = §1.3 调用方实际所需：`create / kill / exists / list / write / resize / subscribe_output / capture_screen / pane_title / current_cwd / is_active / agent_snapshot`。两个实现：
  - `TmuxEngine`：**纯包装现有 `src/tmux/` 代码，行为零变化，此后冻结**（只修致命 bug，不加功能）；v6 起为**长期维护态**，无摘除时间表。
  - `PtyEngine`：新建（Phase 2+），**新建会话的默认选中引擎**（用户可改选 tmux）。
- **按会话路由**：`runtime_kind` 决定引擎（`'tmux'` → TmuxEngine，`'pty'` → PtyEngine；`'acp'` 不经此抽象）。
- ~~**摘除标准（解耦验收）**~~（v6 作废，不再规划摘除）：Phase 1 曾以「删 `TmuxEngine` 目录 + 注册行后 `cargo build` 通过」验证解耦达标（已完成，见 §5）；该能力保留为架构事实，但不作为后续行动依据。
- **否决项**：~~初版 D9：舍弃抽象、一次性切换~~（产品决策要求过渡期）；为 ACP 也套此抽象（ACP 非终端引擎，不强行统一）。
- ~~**翻盘条件**：过渡期后无人使用 tmux 会话且抽象层阻碍 PtyEngine 演进，可提前摘除~~（v6 作废：双引擎长期共存）。
...
### D10（v2 修订，v6 定案）：DB 与命名 —— 最小改动、冻结列名永久保留
- **决策**：`runtime_kind` 新增值 `'pty'`，**新建会话默认选中 `'pty'`**（v6：由 Phase 4 前端选择器呈现，用户可改选 tmux）；存量 `'tmux'` 会话不迁移、继续走 TmuxEngine。新增 `last_cwd` 列（pty 用）。`tmux_session_name` 列**永久不改名**（两引擎共用存引擎内会话 id；v6 无摘除阶段，改名无收益，避免冻结代码 churn）。
- ~~初版：UPDATE 'tmux'→'pty' + 立即改名 engine_session_name~~（v2 作废：违背共存方向）。
...
### D12（新增，v6 修订）：现代化交互按 runtime_kind 分流（长期）
...
- **理由**：双引擎长期共存（v6），交互语义不可混用（AGENTS §8：差异显式降级，不为单一实现背书）。
- ~~**摘除时**：tmux 分支代码整体删除~~（v6 作废：tmux 交互分支长期保留）。
...
- **完成记录（2026-08-09）**：按上述清单落地，187 测试全过、clippy/fmt 零警告。两点执行偏差记录：
  1. **验收口径细化**：`rg tmux`（大小写敏感）边界外残留 = ① `src/engine/mod.rs` 注册行/门面分发（`pub mod tmux;`、WS attach 分发、Windows workaround 门面）；② 冻结持久化/wire 契约——DB 列 `tmux_session_name`、DB 值 `'tmux'`、路由串 `/system/tmux/mouse` 与 `/ws/terminal/external/{tmux_name}`、adopt 请求字段（已 serde rename 中性化）。~~Phase 5 摘除时 ①②一并处理~~（v6 作废：①②作为双引擎共存的冻结契约长期保留）。
...
- **完成记录（2026-08-12）**：三切片独立提交、独立验收：
  - **切片 A**：`PtyEngine` 实现 `SessionEngine`，会话 map 常驻持有 pty 进程；WS 断开只解绑订阅；attach 同锁「先快照后订阅」保证补屏与增量不重不漏；子进程退出自动注销、下次 attach 重建；三级信号清理 + child 收割（`mem::forget` 已移除）。引擎级验收（断开→重连存活可输入）+ 真实 WS 冒烟通过；fd 计数/spawn cwd OS 真相回归测试就位。
   - **切片 B**：spike 结论——wezterm-term 以 git 依赖锁 tag `20240203-110809-5046fc22` 可拉取、全树编译通过（D8 翻盘条件不触发）；`vt.rs` 提供 feed/capture/title/resize，capture 走 VT grid 消除 ANSI 碎片；重连 resize nudge 落地（`reconnected` 标志）。**执行偏差**：补屏未采用「模拟器重渲染整帧」，沿用补屏环原始 ANSI 回放——前端 xterm.js 消费原始 ANSI 流，字节回放保真度更高；herdr 的帧重渲染适配其帧 diff 协议，对 ANSI 流客户端无增益（记录在 `vt.rs` 头注）。
     ...
     - **勘误 2（2026-08-22 同日）：重连 resize nudge 移除**（herdr 借鉴项翻盘）。grid 整帧渲染落地后用真实字节流离线复现实测：alacritty 对 rows shrink→expand **非内容中性**——屏幕整体上滚一行、顶部混入 scrollback 残片（`terminal_ws.rs` 注释与 mod.rs `reconnect_replay_frame_reproduces_screen_exactly` 回归测试为证）。nudge 在新机制下从「救花屏」变成「污染补屏真相源 + 让后续增量永久错位」：同尺寸重连补屏帧已精确无需重绘；变尺寸重连由真实 resize 触发内核 SIGWINCH，全量重绘型程序自然重绘。herdr 场景差异：其客户端消费帧 diff 协议、无服务端 grid 真相源可依，nudge 对它是必要补偿；本项目有 grid 整帧渲染后即冗余。
...
### Phase 4：前端双引擎可选 + 交互分流（v6 修订：原「切换默认 + 未来摘除」作废）
> v6 方向：**pty 与 tmux 长期共存**。创建会话时用户显式选择引擎（pty 为默认选中项，tmux 为长期保留的维护态选项）；后端创建分支已按 `runtime_kind` 双路就绪，本阶段主体是前端。
- **后端**：收尾对齐，不新增能力——
  - `RuntimeKind::default()` 语义核查：API 未传 `runtime_kind` 时的落点须与前端默认一致（pty）；`'acp'` 仍由 agent 会话专用。
  - `ws/terminal.rs` pty 路径与 `api/files.rs` pty 路径已随 Phase 2/3 就绪（重建/回放/补屏/nudge、`last_cwd`/实时采样），本阶段只做双引擎回归。
- **前端**（D12 分流，tmux 分支长期保留不删）：
  - `api/client.ts`：`runtime_kind: 'tmux' | 'pty' | 'acp'`。
  - 创建会话入口（`CreateSessionModal` / Sidebar）：新增引擎选择器——pty（默认）/ tmux，各带一句话差异说明（pty：后端重启自动重建、纯左键拖选复制；tmux：后端重启幸存、prefix 键位与 copy-mode）；无 tmux 环境（`/system/multiplexer` 不可用）时选项禁用并提示。
  ...
  - `Sidebar.tsx`：external 区块仅在系统有 tmux 时展示（维护态能力）。
...
- **通用**：
  ...
  - ~~模拟摘除演练：本地删 `src/engine/tmux/` + 注册行，`cargo build` 通过（解耦达标证明，不提交）~~（v6 作废：Phase 1 已完成该演练并证明解耦达标，不再规划摘除）。
...
**实施后更新**：`docs/architecture/backend.md`（引擎抽象 + 差异表）、`docs/architecture/frontend.md`（D12 分流）、`docs/workflows/agent-edit-manual.md`、AGENTS.md（tmux 冻结说明）、`CHANGELOG.md`、`PROGRESS.md`、`docs/reference/user-testing.md`（双引擎用例）；`docs/reference/herdr-reference.md` 已增补。~~Phase 5 执行时本文件与方向规划移入 `archive/`~~（v6 作废：双引擎长期共存，本文件与方向规划作为长期架构依据保留）。
