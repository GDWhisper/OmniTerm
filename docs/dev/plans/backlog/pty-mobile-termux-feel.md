...
| 亚行余量 | `wheelAccumLines` 余量累积 | `viewportController.ts:174-186` | ✓ 同构（Termux `mScrollRemainder` 同为**整数行**累积，**无**亚行像素偏移 —— 见 §9 E-1） |
| **惯性** | **无**，抬手即停 | — | ✗ 缺失 |
...
### 3.2 实测结果（2026-08-28，探针 `scripts/pty-viewport-probe.mjs`）

探针直连后端 WS（100×40 pty 会话，`hello` 握手启用 cell_frame），**不经浏览器**；跑法 `node scripts/pty-viewport-probe.mjs`（需 `./dev.sh start`）。

| 指标 | 实测值 | 结论 |
|---|---|---|
| 单帧体积 | 94.4 KB（源码 / 数字）、98.5 KB（彩色 ls） | 落在估算上沿 |
| 帧体积 vs 步长 | Δy=1 与 Δy=40 **同为 94.4 KB** | 整屏全帧：滚 1 行也传 40 行，94% 冗余 |
| 请求→响应（本地回环） | p50 **10.2 ms**、p95 12.9 ms | **推翻"本地 RTT <1ms"**（方案 C D2 勘误 2 的假设） |
| 吞吐 | 16 ms 间隔连发 60 个请求 → 60 个响应，**零丢失** | 后端有界通道（容量 4）未成瓶颈；瓶颈是**延迟**不是吞吐 |
| 实时 diff 帧（对照） | 平均 150 B–4.5 KB/帧 | 实时路径有 diff，**历史窗口路径没有** → 不对称是帧体积的根源 |

**瘦身/压缩收益**（同一帧，按内容类型实测）：

| 内容 | 原始 | gzip | RLE（行内按 sgr 合并连续字符） |
|---|---|---|---|
| 数字 seq（高重复） | 94.4 KB | 0.69 KB（137×） | 4.4 KB（21.6×） |
| 彩色 ls（中重复） | 98.5 KB | 2.05 KB（48×） | 5.1 KB（19.5×） |
| 源码 cat（低重复） | 94.4 KB | 1.83 KB（52×） | 4.4 KB（21.6×） |

**permessage-deflate 路径不存在**：`tungstenite 0.29` 已移除 `deflate` feature（`tungstenite-0.29.0/Cargo.toml` features 表无该项），axum 0.8 亦不透出压缩选项（`axum-0.8.9/src/extract/ws.rs` 无压缩相关代码）。**压缩只能做在应用层**。

**推论（改变分期优先级，见 §9 E-5）**：RLE 收益 20× 且稳定（不随内容类型波动），同时把服务端**编码量**降 20 倍（编码耗时是 10 ms 延迟的主因）；gzip 再高 2–6 倍，但要引入二进制帧类型标记 + 前端 `DecompressionStream` 降级路径。二者都能把"预取一屏"的成本从 94 KB 降到个位数 KB —— **这让"预取 + 本地滚动"从昂贵变为可行**。

**假设修正**：方案 C D2 的"本地 WS RTT <1ms"（`pty-herdr-style-full-buffer-render.md` 勘误 2）被实测推翻 —— **即便本地回环也要 10 ms 量级**（主因是服务端编码 94 KB JSON），移动端的网络 RTT（4G 30–60 ms、3G 100 ms+）还要叠加其上。因此"本地往返足够快，无需前置数据"的推论不成立。
...
### 3.3 改动后复测（2026-08-28，P1 实施完成）

P1（帧 RLE）已实施，实施计划、实测与偏差见
`docs/dev/plans/archive/2026-08-28-pty-frame-rle.md`。同探针、同环境（100×40），两种编码
各跑一轮（`ROW_ENCODING=cells` 跑旧格式对照）：
...
| 请求→响应 p95 / max | 15.87 / 15.93 ms | 5.38 / **6.07 ms** | 3.0× / 2.6× |
| 实时 diff 帧平均 | 3758 B | **409 B** | 9.2× |
| 无损性 | — | 四类内容 PASS | 双连接交叉比对 |
| 吞吐 | 零丢失 | 零丢失 | 持平 |

**帧瘦身后还暴露出两个独立的延迟源，已一并修掉**（实施计划 §10.2 / E-7 / E-8）：

1. **diff 行指纹的每帧 4000 次堆分配**。`encode_cell_frame` 的 diff 路径要先为整行
   构造 `CellData`（每 cell 一次 `sgr_body` 的 String + 一次 `to_string`），40×100 屏
   即 4000 次分配/帧。RLE 压下行编码成本后，这笔开销成了实时帧的主要成本（30fps
   tick 实测 7.5 ms/帧，占转发循环 23% 的时间）。改为直接遍历 grid 计算指纹后，
   慢帧告警从 405 次/轮降到 2 次。
2. **Nagle + Delayed ACK**。WebSocket 每 33 ms 发一个 ~200 B 的实时小帧，Nagle 把它
   攒到前一个包的 ACK，与对端 40 ms 的 Delayed ACK 叠加 → 尾延迟 ~50 ms。给 accept
   的每个连接开 `TCP_NODELAY` 后，max 从 50 ms 降到 6.07 ms。

定位线索是「把 30fps tick 临时改成 1000 ms，尖峰即消失」——但 tick 只是**触发**
条件而非根因：只有它持续产生小包。真正的两个根因都在上面，与 tick 本身无关。

**推论更新**：帧体积已降到 4.8 KB、p50 2.84 ms、尾延迟 6 ms，**「预取一屏 + 本地
滚动」（A2/A4/A5）的可行性假设成立**，可以进入 P2 的评估。

**遗留的结构弱点**：转发循环是单个 `select!`，分支内 `send().await` 一旦被背压阻塞
仍会拖住其余分支（当前帧已小到几乎不触发）。tick 分支留了 `SLOW_FRAME_US` 告警哨兵。
...
| Phase | 内容 | 独立可验证 | 依赖 |
|---|---|---|---|
| **P0** | 惯性：触摸速度采样 + Scroller 衰减（rAF） | 抬手后继续滚并减速 | 无（纯前端，不动数据模型） |
| **P1** | 帧瘦身：RLE 行内编码 → **已实施**（2026-08-28，计划与实测见 `docs/dev/plans/archive/2026-08-28-pty-frame-rle.md` §10） | 单帧 94.4 KB → **4.8 KB**（19.8×）、RTT p50 12.75 → 2.84 ms，四类内容无损 PASS | 无（与 P0 独立，可并行） |
| **P1.5** | 实施期暴露的两个延迟源：① diff 行指纹的每帧 4000 次堆分配 ② Nagle + Delayed ACK 的 ~40 ms 尾延迟 → **已随 P1 一并修掉**（§3.3） | 30fps 实时帧 7.5 ms/帧 → 阈值以下；RTT max 50 → 6.07 ms | 由 P1 的实测暴露 |
| **P2** | 本地滚动窗口：预取 N 屏 + 窗口内零 IO 滚动 | 快速滑动内容不空白、不滞后 | P1 + P1.5（预取要既便宜又跟手） |
| **P3** | 键行 Termux 化（popup + 长按重复 + 配置驱动） | 上滑出第二功能；长按连续发键 | 无（与 P0/P1 并行） |
| **P4** | 可选：应用层 gzip 叠加 / 后端按行区间供给 | 单帧再降 2–6 倍 | P2 后按实测再定 |

> **亚行 transform 已从 P0 移出**：Termux 本身没有亚行像素偏移（§9 E-1），它属于"超出 Termux 的自选增强"，且需要 over-allocate 才不露白、有交互坐标风险。若 P2 落地后仍需更极致的跟手感，再单独立项评估。

P0 单独做即能拿到大部分"跟手 + 惯性"收益，且不动后端与数据模型。P3 与滚动无关，可独立排期（已决策与 A 并行，见 §9 D-1）。

> **P1/P2/P4 是否实施取决于实测结论**：若实测显示移动端帧往返能跟上快速滑动，则滚动改造止于 P0；跟不上再上 P1 → P2/P4。**P1 已按此结论实施完毕**（§3.3），P1.5 是实测暴露出的下一项。
...
### 勘误（2026-08-28 实测后，就地记录）

**E-1 Termux 没有亚行像素偏移 —— §2.2「亚行精度」要素被高估**
`mTopRow` 是 `int`（`TerminalView.java:70`），`render(..., int topRow, ...)` 按整数行循环绘制（`TerminalRenderer.java:57-73`），无 `canvas.translate` 小数偏移；`mScrollRemainder` 仅用于手势累积（`:180-183`）与抬手清零（`:144`）。即 Termux 的"亚行余量"= 不足一行的手势距离不丢弃，**与我们的 `wheelAccumLines` 完全同构**。
→ 影响：亚行 transform 不是对齐 Termux 的必要项；Termux 的跟手来自**零 IO 本地重绘**（改 int → `invalidate()`），这才是要对标的核心。

**E-2 "本地 RTT <1ms" 假设不成立**
实测本地回环 p50 = 10.2 ms（§3.2），主因是服务端编码 94 KB JSON 的耗时，与传输距离无关。方案 C D2 据此假设推演的"无需本地数据"结论一并失效。

**E-3 帧体积与滚动步长无关**
Δy=1 与 Δy=40 同为 94.4 KB —— 每次滚 1 行都重传整屏，冗余 94%。

**E-4 实时路径与历史窗口路径不对称**
实时帧走 diff（平均 150 B–4.5 KB），窗口帧是 `full:true` 整屏全帧。这是帧体积问题的直接根源，也是 RLE 之外另一个可选方向（给窗口帧也上 diff / delta 行供给）。

**E-5 A4/A5 的定位重排**
原定"预渲染窗口（A2）+ 按行区间供给（A4）+ 前端行缓存（A5）"的重路线，其成本主要来自"预取一屏要 94 KB"。**帧瘦身（P1）把该成本降到个位数 KB**，于是：
- 预取从昂贵变可行，A2 可在瘦身之后以更低的 K（更少的 refill 成本）实现；
- A5 的行缓存上界问题同步简化（按内存定界即可，不必为带宽砍到 1000 行）；
- permessage-deflate 不可行（tungstenite 0.29 无 deflate feature），应用层 gzip 列为 P4 可选项而非前提。
