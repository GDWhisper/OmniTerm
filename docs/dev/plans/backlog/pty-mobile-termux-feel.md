...
### 3.3 改动后复测（2026-08-28，RLE 实施完成）

P1（帧 RLE）已实施，实施计划与细节见 `docs/dev/plans/2026-08-28-pty-frame-rle.md`。
同探针、同环境（100×40）复测：
...
| 请求→响应 max | 21 ms | **50 ms** | **变差，见下** |
| 无损性 | — | 四类内容 PASS | 双连接交叉比对 |
| 吞吐 | 零丢失 | 零丢失 | 持平 |

**新的延迟瓶颈已不在帧体积**：体积降 19.8× 而 RTT 只降 2.6×。定位过程（实施计划
§10.2）——把 30fps 实时帧 tick 临时从 33 ms 改成 1000 ms 后，runs 的 p50 降到
2.48 ms、max 降到 2.67 ms，尖峰完全消失：

| 条件 | runs p50 | runs max |
|---|---|---|
| tick = 33 ms（现状） | 4.83 ms | 49.95 ms |
| tick = 1000 ms（仅实验） | 2.48 ms | 2.67 ms |

根因是 `src/engine/pty/terminal_ws.rs` 转发循环用单个 `tokio::select!` 串行处理
多路输出，tick 分支与 viewport 分支都在分支内 `ws_tx.send(..).await` —— 发送一旦
因背压阻塞，整个循环停住，期间到达的 viewport 请求排队（尖峰与 tick 同周期，
占样本 ~7.5%）。**这是独立于帧瘦身的第二个延迟源，需另立项**（纳入 §5 分期）。

推论更新：帧瘦身后「预取一屏」成本已是 4.8 KB 级别，**本地滚动窗口（A2/A4/A5）
的可行性假设成立**；但先解掉 tick 争用才能让预取真正跟手。
...
| **P1** | 帧瘦身：RLE 行内编码 → **已实施**（2026-08-28，计划与实测见 `docs/dev/plans/2026-08-28-pty-frame-rle.md` §10） | 单帧 94.4 KB → **4.8 KB**（19.8×），四类内容无损 PASS | 无（与 P0 独立，可并行） |
| **P1.5** | **新增**：解 30fps tick 与 viewport 响应的争用（转发循环单 `select!` 串行 + `send().await` 阻塞） | RTT p50 4.8 → ~2.5 ms，max 50 → ~3 ms（已在实验中验证） | P1 后的下一个延迟瓶颈，见 §3.3 |
