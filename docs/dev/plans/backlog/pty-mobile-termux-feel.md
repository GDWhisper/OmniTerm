...
| **P1** | 帧瘦身：RLE 行内编码 → **已立项，实施计划见 `docs/dev/plans/2026-08-28-pty-frame-rle.md`** | 单帧 94 KB → ~4.4 KB；服务端编码量同比例下降 | 无（与 P0 独立，可并行） |
| **P2** | 本地滚动窗口：预取 N 屏 + 窗口内零 IO 滚动 | 快速滑动内容不空白、不滞后 | P1（预取成本依赖瘦身） |
