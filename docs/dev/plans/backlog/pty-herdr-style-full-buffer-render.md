...
2. 在 scrollback 中看到新输出 → 自动弹回底部（与 WezTerm/VSCode 终端一致）
   ...
   >
   > **勘误 2（2026-09-03）：上条的 ① 已证伪，锚定改由后端按内容指纹完成。**
   > 「前端拿 `history_size` 反推绝对行号」实测仍在漂移：`history_size` 取自主
   > 线程收到的上一帧（30fps，最坏落后 33ms），叠加 rAF 延迟后每次重拉都漂
   > ~0.5 行（20 行/秒输出 → 视图以 5.6 行/秒滑动，达输出速率 28%；100 行/秒
   > → 20 秒漂 109 行）。更根本的是：**历史饱和（1000 行）后 `history_size`
