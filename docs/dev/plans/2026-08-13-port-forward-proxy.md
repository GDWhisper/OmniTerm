...
> 6. **路径前缀兜底 = 响应体重写（2026-08-14）**：D1 翻盘为子域名方案后发现其依赖可通配符解析的域名，**局域网纯 IP 直连（`http://192.168.5.216:9777`）不可用**（`3000.192.168.5.216` 非法）；Service Worker 方案又需 secure context（HTTPS/localhost），局域网 HTTP 也不可用。故路径前缀形态新增**响应体字节级重写**兜底：`text/html` 的 `src`/`href`/`srcset`/`action`/`poster` 属性与 `text/javascript` 的 `"/api/`、`'/api/`、`` `/api/ `` 字面量统一补 `/proxy/{port}` 前缀（HTML 4MiB / JS 16MiB 上限，超限回退流式）。**全局一致重写**保证 `===` 比较逻辑自洽；JS 运行时动态拼接路径覆盖不到，属已知限制。覆盖「绝对路径 SPA 白屏」的局域网纯 IP 场景，与子域名方案互补（子域名管有域名场景，重写管纯 IP 场景）。
   ...
   > 8. **SPA 客户端路由适配（2026-08-15 二次勘误）**：3000 端口 SPA（new-api）经代理打开渲染导航栏后主体 404——React Router 创建 history 时读 `window.location.pathname = /proxy/3000/`（带前缀），而路由表只有 `/`、`/console` 等（不带前缀），匹配失败。**首版方案（HTML 内联 polyfill 劫持 `Location.prototype.pathname` + `history.pushState`）实测失败且行为退化**：Chrome 的 `Location` 是 [LegacyUnforgeable] 特殊对象——`window.location` 实例的 `pathname` 属性 `configurable: false` 不可重定义，`Location.prototype` 上的 defineProperty 不影响实例读取，劫持完全无效；且 polyfill 内联在 `<!doctype html>` 前触发 quirks mode，pushState 劫持加前缀后 React Router 从 `window.location.pathname` 读到带前缀路径 → 点击导航也 404（修复前点击还能跳转，修复后反而全坏）。**正解 = JS 重写注入 React Router v7 `basename`**：`<BrowserRouter future={{v7_startTransition}}>`（v7 独有特征）props 注入 `basename:"/proxy/{port}"`，React Router 自动剥前缀匹配、Link/navigate 自动带前缀、地址栏正确、刷新正常。子域名方案仍为根治（无需 SPA 端配合），本修复让纯 IP 场景也能用。另修复 Vite 8 preload helper（`return"/"+e` 硬编码根绝对路径）导致 chunk MIME 404，改写为 `return"/proxy/{port}/"+e`。
