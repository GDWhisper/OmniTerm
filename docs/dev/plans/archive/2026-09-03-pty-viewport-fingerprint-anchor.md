...
> 状态：已实施（2026-09-03）；**D1-D5 机制已被 2026-09-12 有状态锚取代，见文末勘误**（「y 不是稳定标识」结论仍是后续方案的前提）
...
- **D3「重复行误匹配 → 视觉等价，无害」在连续刷新下不成立**（本次推翻的是
  09-04 勘误「仅对 y>0 成立」的保留判断）：该判断只对**单次**刷新成立——窗口
  首行内容确实相同；但重定位起点由滞后的 y 反推、落在真锚点靠 live 一侧 k 行
  处，历史中周期为 p 的同内容副本距起点仅 `p − k` 行，**p < 2k 时必先命中更新
  的副本**，每轮刷新向 live 棘轮推进，累积即用户滚动位置被系统性擦除。空行带
  （p=2，Ink 系 agent 输出常态）、框线、分隔线均实测复现。
- **「内容回答的是这行在哪，用户要的是我看的那个位置在哪」**：锚定行内容重复
  时指纹在原理上无解（邻域指纹、单侧搜索、限幅护栏均被推演否决，见 09-12 计划
  方案选型）。位置记忆必须驻留在持有 grid 真相源的一侧（`VtState.viewport_anchor`
  绝对行索引），刷新路径不存在「重新决定位置」的步骤。
- 09-04 y=0 勘误（跳过重定位）由新方案的「y=0 清锚」自然承接；
  `viewport_frame_y0_ignores_fingerprint_even_when_history_matches` 等指纹用例
  随机制一并删除，新用例组见 09-12 计划验收清单。
