...
4. **合并前检查源分支未提交代码** — 执行 `git merge <branch>` 前，必须检查源分支 worktree 是否有未提交的修改（`git status --short`），特别是与目标分支有交集的模块。未提交的代码不会随 merge 搬运，遗漏可能导致目标分支运行时异常。如存在相关未提交代码，需先提交或与用户确认处理方案后再合并。
