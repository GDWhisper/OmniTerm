...
pub async fn ws_terminal_handler(
    ...
    ws.on_upgrade(move |socket| async move {
        ...
        match runtime_kind {
            Some((kind,)) if kind == "pty" => {
                handle_pty_terminal(socket, session_id, query, state).await
            }
            _ => handle_terminal(socket, session_id, query, state).await,
        }
    })
}
