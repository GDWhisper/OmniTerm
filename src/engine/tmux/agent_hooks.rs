//! Agent hook 注入（复用器 option 信道版）：为 agent 命令生成把状态写回
//! `@omniterm_agent` 会话 option 的 hook 配置。CLI 识别在 `crate::agent::cli`。
...
///
/// Returns a JSON string that maps each hook event to a command that writes
/// agent state to the `@omniterm_agent` session option.
...
mod tests {
    ...
    // === shell escaping tests (via clean_token in agent state) ===

    #[test]
    fn test_shell_escaping_special_chars() {
        use crate::agent::state::clean_token;
